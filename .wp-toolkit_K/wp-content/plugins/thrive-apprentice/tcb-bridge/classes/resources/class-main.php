	...
	public function lesson_resources( $attr, $content ) {
		...
		if ( ! empty( $attr['data-lesson'] ) ) {
			...
			if ( ! empty( $course ) ) {
				...
				$course_published = $course->is_published() || $course->is_hidden();
				$lesson_published = $lesson->is_published();
				$course_visible   = $course_published || $course->is_archived();
				$has_access       = ( $course_visible && $lesson_published && tva_access_manager()->has_access_to_object( $lesson->get_the_post() ) ) || tva_is_preview();
				...
				if ( ( $has_access && ! empty( $tva_shortcode_lesson_resources ) ) || self::$is_editor_page || ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
					$should_render = true;
						if ( self::$is_editor_page ) {
							...
							} elseif ( ! $course_published && ! $course->is_archived() && ! $course->is_hidden() ) {
