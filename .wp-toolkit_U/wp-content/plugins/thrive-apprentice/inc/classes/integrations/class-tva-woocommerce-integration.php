...
class TVA_Woocommerce_Integration extends TVA_ThriveCart_Integration {
	...
	public function process_order( $order_id, $status_from, $status_to, $wc_order ) {
		...
		/**
		 * If order transitions to completed, align TVA timestamps to the WooCommerce
		 * completion moment so that expiry/drip "after purchase" logic starts when
		 * access is actually granted (e.g., Direct Bank Transfer orders).
		 */
		if ( 'completed' === $status_to ) {
			$completed_dt = $wc_order->get_date_completed();
			$created_at   = $completed_dt instanceof \WC_DateTime ? $completed_dt->date( 'Y-m-d H:i:s' ) : current_time( 'mysql' );

			$tva_order->set_created_at( $created_at );
			$tva_order->set_updated_at( $created_at );

			foreach ( $tva_order->get_order_items() as $order_item ) {
				if ( $order_item instanceof TVA_Order_Item ) {
					$order_item->set_created_at( $created_at );
				}
			}
		}
		...
		// Trigger events for online payments immediately; for offline payments only on completion
		$payment_method          = $wc_order->get_payment_method();
		$offline_payment_methods = array( 'bacs', 'cheque', 'cod' ); // Bank transfer, Check, Cash on Delivery
		$is_offline_payment      = in_array( $payment_method, $offline_payment_methods, true );
		$should_trigger_events   = ! $is_offline_payment || 'completed' === $status_to;

		if ( $should_trigger_events && ! empty( $enrolled_courses ) ) {
			$customer->trigger_course_purchase( $tva_order, 'WooCommerce' );
		}

		if ( $should_trigger_events && ! empty( $all_order_products ) ) {
