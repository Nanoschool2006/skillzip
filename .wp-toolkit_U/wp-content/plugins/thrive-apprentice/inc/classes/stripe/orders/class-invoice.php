...
class Invoice extends Checkout {
	...
	public function process_data() {
		...
		list( $first_name, $last_name ) = Helper::get_names( $this->data->customer_email );
		$user = $this->get_user( $this->data->customer_email, $first_name, $last_name );

		if ( $user ) {
			$this->order->set_user_id( $user->ID );
			$customer = $this->data->customer;
			if ( ! $customer ) {
				$customer = Request::get_customer_id( $this->data->customer_email );
			}
