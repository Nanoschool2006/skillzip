...
class Request {
	...
	public static function get_customer_id( $email ) {
		...
		if ( empty( $connection ) || empty( $connection->get_client() ) ) {
			return false;
		}
	...
	/**
	 * Search products by tapp_product_id (TA Product ID).
	 *
	 * @param int $product_id The ID of the product.
	 * @param bool   $test_mode  Whether to use test mode.
	 *
	 * @return WP_Error|array The products or a WP_Error object if an error occurred.
	 */
	public static function search_products( $product_id, $test_mode ) {
		$products   = [];
		$connection = Connection_V2::get_instance();
		$connection->set_test_mode( $test_mode );
		$client = $connection->get_client()->products;

		$filters = [
			'query' => 'active:\'true\' AND metadata[\'tapp_product_id\']:\'' . $product_id . '\''
		];

		try {
			$products = $client->search( $filters, [
				'stripe_account' => Credentials::get_account_id(),
			] );
		} catch ( ApiErrorException $e ) {
			return new WP_Error( 'stripe_error', $e->getMessage() );
    ...
    public static function get_product( $product_id, $test_mode = false ) {
	...
	public static function create_product( $product_data, $test_mode = false ) {
		...
		try {
			return $client->products->create( $product_data );
		} catch ( ApiErrorException $e ) {
			return new WP_Error( 'stripe_error', $e->getMessage() );
		}
	}

	public static function update_product( $product_id, $product_data, $test_mode = false ) {
		...
		try {
			return $client->products->update( $product_id, $product_data );
		} catch ( ApiErrorException $e ) {
			return new WP_Error( 'stripe_error', $e->getMessage() );
		}
	}
	...
	public static function create_product_price( $price_data, $test_mode = false ) {
		...
		try {
			return $client->prices->create( $price_data );
		} catch ( ApiErrorException $e ) {
			return new WP_Error( 'stripe_error', $e->getMessage() );
		}
	}

	public static function update_product_price( $price_id, $price_data, $test_mode = false ) {
		...
		try {
			return $client->prices->update( $price_id, $price_data );
		} catch ( ApiErrorException $e ) {
			return new WP_Error( 'stripe_error', $e->getMessage() );
		}
	}
	...
	/**
	 * Enables the customer portal.
	 *
	 * This method enables the customer portal by ensuring the customer portal configuration
	 * and performing necessary hooks to ensure the customer portal is functional.
	 *
	 * @return mixed Returns true if the customer portal is enabled successfully, error message otherwise.
	 */
	public static function enable_customer_portal() {
		$connection   = Connection_V2::get_instance();
		/**
		 * Ensure the customer portal configuration for live mode.
		 */
		$response = $connection->ensure_customer_portal_configuration();
		
		/**
		 * Ensure the customer portal configuration for test mode.
		 */
		$response = true === $response ? $connection->ensure_customer_portal_configuration( false ) : $response;

		if ( true === $response ) {
			Hooks::ensure_customer_portal();
			Hooks::ensure_customer_ids();
		}
