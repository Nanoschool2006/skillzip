...
class TGE_Question_Manager {
	...
	public function get_quiz_questions( $filters = array(), $single = false ) {
		...
		if ( ! empty( $filters['with_answers'] ) ) {
			foreach ( $questions as &$question ) {
				$question['position']         = ! empty( $question['position'] ) ? json_decode( $question['position'] ) : null;
				$question['settings']         = ! empty( $question['settings'] ) ? json_decode( $question['settings'] ) : null;
				$question['display_settings'] = ! empty( $question['display_settings'] ) ? json_decode( $question['display_settings'] ) : null;
	...
	public function get_first_question_preview( $question_data = array(), $quiz_id = 0 ) {
		...
		if ( ! empty( $quiz_id ) ) {
			...
			$has_next_button    = $navigation_enabled || $feedback_settings['press_next'] || $is_openended || ( isset( $settings['allowed_answers'] ) && $settings['allowed_answers'] > 1 );
		} else {
	...
	public function tqb_build_video_frame( $display_settings ) {
		...
		$embed_urls     = array(
			...
			'bunnynet'   => 'https://iframe.mediadelivery.net/embed/',
			'custom'  => '',
		);
		$source         = ! empty( $display_settings->source ) ? $display_settings->source : 'custom';
		$video_id       = ! empty( $display_settings->video_id ) ? $display_settings->video_id : '';
		$library_id       = ! empty( $display_settings->library_id   ) ? $display_settings->library_id   : '';
		$video_url = ('custom' === $source) ? $display_settings->url : (('bunnynet' === $source) ? $embed_urls[$source] . $library_id . '/' . $video_id : $embed_urls[$source] . $video_id);
	...
	public function save_question( &$question ) {
		...
		// Invalidate quiz map cache when question is saved
		if ( ! empty( $this->quiz_id ) ) {
			global $tqbdb;
			if ( $tqbdb ) {
				$tqbdb->invalidate_quiz_map_cache( $this->quiz_id );
			}
		}
	...
	public function delete_question( $id ) {
		...
		// Invalidate quiz map cache when question is deleted
		if ( ( $q_deleted || $a_deleted ) && ! empty( $this->quiz_id ) ) {
			global $tqbdb;
			if ( $tqbdb ) {
				$tqbdb->invalidate_quiz_map_cache( $this->quiz_id );
			}
	...
	public function save_answer( &$answer ) {
		...
		// Invalidate quiz map cache when answer is saved
		if ( ! empty( $this->quiz_id ) ) {
			global $tqbdb;
			if ( $tqbdb ) {
				$tqbdb->invalidate_quiz_map_cache( $this->quiz_id );
	...
	public function get_min_max( $q ) {
		...
		if ( $this->is_leaf( $q ) ) {
			...
			$this->cache_min_max[ $q['id'] ] = [
				'min' => ! empty( $points ) ? min( $points ) : 0,
				'max' => ! empty( $points ) ? $this->get_max_points( $points, $q ) : 0
			];
		...
		if ( empty( $answer_points ) ) {
			$this->cache_min_max[ $q['id'] ] = array( 'min' => 0, 'max' => 0 );
			return $this->cache_min_max[ $q['id'] ];
		}
	...
	public function get_max_points( $points, $question ) {
		// Return 0 if points array is empty
		if ( empty( $points ) ) {
			return 0;
		}

		// for right_wrong quiz type we don't take into consideration multiple answers
		$allowed_answers = $question['allowed_answers'] ?? 1;
		if ( $this->quiz_type !== Thrive_Quiz_Builder::QUIZ_TYPE_RIGHT_WRONG && $allowed_answers > 1 && count( $points ) >= $allowed_answers ) {
			...
			return array_sum( array_slice( $points, 0, $allowed_answers ) );
		}
