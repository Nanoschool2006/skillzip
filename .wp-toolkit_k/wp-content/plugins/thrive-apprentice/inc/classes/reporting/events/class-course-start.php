...
class Course_Start extends Event {
	...
	public static function label(): string {
		return esc_html__( 'Course start', 'thrive-apprentice' );
	}

	public static function get_tooltip_text(): string {
		return '<strong>{number}</strong> ' . esc_html__( 'courses started', 'thrive-apprentice' );
	}
	...
	public static function register_action() {
		add_action( 'thrive_apprentice_course_start', static function ( $course_details, $user_details ) {
			$event = new static( [
	...
	public function get_event_description(): string {
		...
		return sprintf( esc_html__( ' started course %s.', 'thrive-apprentice' ), $item );
	}
}
