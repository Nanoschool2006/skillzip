...
class Thrive_Theme_List {
	...
	public function render() {
		...
		if ( empty( $items ) ) {
			...
			$this->attr['icon-markup'] = $this->attr['icon-markup'] ?? Thrive_Shortcodes::get_icon_by_name( $this->attr['icon'] ?? '' );

			$content = Thrive_Utils::get_element( 'dynamic-list', [
				'icon-markup' => $this->attr['icon-markup'],
				'items'       => $items,
			], false );
		}
		...
		/* Add element type attribute for proper identification in the editor */
		$this->attr['tcb-elem-type'] = 'thrive_dynamic_list';
