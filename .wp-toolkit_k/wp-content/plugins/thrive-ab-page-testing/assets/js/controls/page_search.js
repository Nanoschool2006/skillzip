	...
	bind: function () {
		...
		this.$autocomplete.autocomplete( {
			...
			source: function ( request, response ) {
				// Check if the input looks like a URL or permalink
				...
				// - Paths starting with / (but not just a single /)
				var isUrl = /^(https?:\/\/|\/\/|www\.|[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}|\/[^\/\s])/.test( term );
				
				if ( isUrl ) {
					// Show helpful message instead of searching
					var helpMessage = {
						title: 'Search by page name, not URL',
						label: 'Search by page name, not URL. Try "About Us" instead of "' + term + '"',
						type: 'Help',
						value: '',
						isHelpMessage: true
					};
			...
			select: function ( event, ui ) {
				// Prevent selection of help messages
				if ( ui.item.isHelpMessage ) {
					return false;
				}
		...
		this.$autocomplete.data( 'ui-autocomplete' )._renderItem = function ( ul, item ) {
			...
			// Special styling for help messages
			if ( item.isHelpMessage ) {
				$li.addClass( 'ui-help-message' );
			...
			if ( item.isHelpMessage ) {
				_html = '<span class="post-name help-message">' + item.label + '</span>';
			} else if ( item.type === 'Create New Page' ) {
