...
class TVA_Courses_Controller extends TVA_REST_Controller {
	...
	public function register_routes() {
		...
		$this->register_routes_automator();
	}
	...
	public function register_routes_v2() {
		...
		/**
		 * GET Course enrollment statistics (async loading for performance)
		 */
		register_rest_route(
			static::$namespace . 2,
			'/' . $this->base . '/stats',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_courses_stats' ),
					'permission_callback' => array( $this, 'courses_permissions_check' ),
					'args'                => array(
						'course_ids' => array(
							'required'          => true,
 ...
 */
	...
	public function register_routes_automator() {
		register_rest_route(
			static::$namespace_public . static::$version,
			'/courses',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
	...
	public function get_courses_v2( $request ) {
		...
		$response = array(
			'items' => TVA_Course_V2::get_basic_items( $args ),
	...
	/**
	 * Get enrollment statistics for courses (loaded asynchronously for better performance)
	 *
	 * @param WP_REST_Request $request Request object.
	 *
	 * @return WP_REST_Response
	 */
	public function get_courses_stats( $request ) {
		...
		// Generate cache key based on course IDs and cache version
		sort( $course_ids ); // Sort to ensure consistent cache key
		$cache_key = 'tva_courses_stats_v' . TVA_Course_V2::get_courses_cache_version() . '_' . md5( implode( '_', $course_ids ) );
		...
		foreach ( $course_ids as $course_id ) {
			...
			if ( $course && $course->term_id ) {
				$stats[ $course_id ] = array(
					'id'                => (int) $course_id,
					'conversions'       => $course->count_conversions(),
					'enrolled_users'    => $course->count_enrolled_users(),
					'count_lessons'     => $course->count_lessons(),
					'count_assessments' => $course->count_assessments(),
				);
			}
		}

		// Cache the results with filterable expiration time
		$cache_expiration = apply_filters( 'tva_courses_stats_cache_expiration', HOUR_IN_SECONDS );
		set_transient( $cache_key, $stats, $cache_expiration );
	...
	public function update_item( $request ) {
		...
		$course->send_welcome_email  = rest_sanitize_boolean( $request->get_param( 'send_welcome_email' ) );
	...
	public function update_orders( $request ) {
		...
		// Clear course cache after order update
	...
	public function get_courses_public ( $request ) {
		...
		foreach ( $courses as $key => $course ) {
			$courses[ $key ]->load_structure();
			$courses[ $key ]->get_overview_post( true );
		}

		return rest_ensure_response( $courses );
	}

	public function get_course_public ( $request ) {
		$id     = (int) $request->get_param( 'id' );
		$course = new TVA_Course_V2( $id );
		if ( ! $course || ! $course->term_id || is_wp_error( $course ) ) {
			return new WP_Error( 'invalid_course', __( 'There is no course with this ID', 'thrive-apprentice' ) );
		}
	...
	public function get_course_items_public ( $request ) {
		$id     = (int) $request->get_param( 'id' );
		$course = new TVA_Course_V2( $id );
		if ( ! $course || ! $course->term_id || is_wp_error( $course ) ) {
			return new WP_Error( 'invalid_course', __( 'There is no course with this ID', 'thrive-apprentice' ) );
		...
		$items = [
			...
			'assessments' => $course->get_published_assessments(),
