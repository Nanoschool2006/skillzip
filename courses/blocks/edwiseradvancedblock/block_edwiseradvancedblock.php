...
class block_edwiseradvancedblock extends block_base {
    ...
    public function get_content() {
        ...
        if (!is_null($this->config)) {
            $templatecontext['blockhtml'] = pre_process_html($this->config->html['text']);
            $templatecontext['blockcss'] = pre_process_css($this->config->css['text'], $this->instance->id);
            $templatecontext['blockjs'] = $this->config->js['text'];
        }
        $this->content->text = "";
    ...
    public function applicable_formats() {
        ...
        $allow = [];
        if (class_exists('local_remuihomepage_plugin') && \theme_remui\toolbox::get_setting('frontpagechooser') == 1) {
    ...
    public function instance_create() {
        ...
        if (!$blockname) {
            $blockname = "blank_html";
        }

        if (is_edwpb_plugin_available()) {
            $bm = new \local_edwiserpagebuilder\block_handler();
            $blockconfig = $bm->get_data_with_title($blockname);
