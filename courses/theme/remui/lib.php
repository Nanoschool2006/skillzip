...
function theme_remui_get_flat_navigation($flatnavigation) {
    global $PAGE, $COURSE, $CFG;

    // This check is to check if we are on course related page.
    if ($PAGE->pagelayout == 'course' || $PAGE->pagelayout == 'incourse' || $COURSE->id != 1) {

        // Course Sections Node Creation
        $coursesectionsmenu = \navigation_node::create(
            get_string('coursesection', 'theme_remui'),
            "javascript:void(0)",
            navigation_node::NODETYPE_BRANCH,
            null,
            'coursesections',
            new pix_icon('i/book', '', 'theme_remui')
        );

        // Course Specific links Node Creation
        $coursespecificmenu = \navigation_node::create(
            get_string('coursespecificlinks', 'theme_remui'),
            "javascript:void(0)",
            navigation_node::NODETYPE_BRANCH,
            null,
            'coursespecificlinks',
            new pix_icon('i/list', '', 'theme_remui')
        );

        // Universal links Node Creation
        $universallinksmenu = \navigation_node::create(
            get_string('universallinks', 'theme_remui'),
            "javascript:void(0)",
            navigation_node::NODETYPE_BRANCH,
            null,
            'universallinks',
            new pix_icon('i/link', '', 'theme_remui')
        );

        // Mycourses links Node Creation
        $mycourseslinksmenu = \navigation_node::create(
            get_string('mycourses', 'theme_remui'),
            "javascript:void(0)",
            navigation_node::NODETYPE_BRANCH,
            null,
            'mycourseslinks',
            new pix_icon('i/graduation', '', 'theme_remui')
        );
        $mycourseslinks = [];
        $coursemenuchilds = [];
        $coursespecificlinks = [];
    ...
    foreach ($flatnavigation as $navs) {
        if (isset($navs->parent->key) && $navs->parent->key == 'mycourses') {
            // Hiding extra navs in flatnavigation
            $navs->hidden = true;
        }

        // Navigation Elements Sorting on Course Page.
        if ($PAGE->pagelayout == 'course' || $PAGE->pagelayout == 'incourse' || $COURSE->id != 1) {
            $cmenus = ['participants', 'badgesview', 'competencies', 'grades'];
            $navs->set_indent(1);
            if (is_numeric($navs->key) && isset($navs->parent->key) && $navs->parent->key == $COURSE->id) {
                // Set the indentation for the nav - which is going to be child of other nav
                $navs->sectionindex = $sectionindex++;

                // Expand the first menu on course page only.
                if ($PAGE->pagelayout == 'course') {
                    $navs->expanded = true;
                    $coursesectionsmenu->expanded = true;
                    $coursesectionsmenu->isactive = true;
                }
                // Set the parent for current nav.
                $navs->set_parent($coursesectionsmenu);
                $coursemenuchilds[] = $navs;

            } else if (in_array($navs->key, $cmenus)) {
                $navs->set_parent($coursespecificmenu);
                if ($PAGE->pagelayout != 'course' && $navs->isactive) {
                    $navs->expanded = true;
                    $coursespecificmenu->expanded = true;
                    $coursespecificmenu->isactive = true;
                }
                $coursespecificlinks[] = $navs;

            } else {
                if (isset($navs->parent->key) && $navs->parent->key == 'mycourses') {
                    if ($navs->key == "courseindexpage") {
                        $navs->action = $CFG->wwwroot . "/course/index.php?mycourses=1";
                    }
                    $navs->set_parent($mycourseslinksmenu);
                    $mycourseslinks[] = $navs;
                } else if ($navs->key !== 'mycourses') {
                    $navs->set_parent($universallinksmenu);
                    $universallinks[] = $navs;
                }
            }
            $navs->hidden = true;
        ...
        // More - nav was not having proper url.
        if ($navs->key == "mycourses") {
            foreach ($navs->children as $key => $child) {
                if ($child->key == "courseindexpage") {
                    $child->action = $CFG->wwwroot . "/course/index.php?mycourses=1";
                }
            }
        }
    }

    $availablekeys = $flatnavigation->get_key_list();

    $addablekeys = ['myhome', 'coursehome', 'home'];

    $addablekey = "";

    foreach ($addablekeys as $key => $value) {
    ...
    // Adding Navigation into Flat Navigation Contex
    if ($PAGE->pagelayout == 'course' || $PAGE->pagelayout == 'incourse' || $COURSE->id != 1) {
        // Add Course Sections Dropdown Menu
        if (!empty($coursemenuchilds)) {
            $coursesectionsmenu->isexpandable = true;
            $coursesectionsmenu->children = $coursemenuchilds;
            $flatnavigation->add($coursesectionsmenu, $addablekey);
        }

        // Add Course Specific Links Dropdown Menu
        if (!empty($mycourseslinks)) {
            $mycourseslinksmenu->isexpandable = true;
            $mycourseslinksmenu->children = $mycourseslinks;
            $flatnavigation->add($mycourseslinksmenu, $addablekey);
        }

        // Add Course Specific Links Dropdown Menu
        if (!empty($coursespecificlinks)) {
            $coursespecificmenu->isexpandable = true;
            $coursespecificmenu->children = $coursespecificlinks;
            $flatnavigation->add($coursespecificmenu, $addablekey);
        }

        // Add Course Specific Links Dropdown Menu
        if (!empty($universallinks)) {
            $universallinksmenu->isexpandable = true;
            $universallinksmenu->children = $universallinks;
            $flatnavigation->add($universallinksmenu, $addablekey);
        }
    }
...
function is_plugin_available($component) {
    ...
    try {
        // This will check if Edwiser Block Manager Plugin is installed - OR throws an exeption
        core_plugin_manager::instance()->plugin_name($component);
    } catch (Exception $ex) {
        return false;
    }
    return true;
