...
class Non_Repeating extends Base {
	...
	/**
	 * Applicable for all trigger types.
	 *
	 * @var string
	 */
	protected $trigger_time = '00:00';
	...
	public function get_next_occurrence( $from_date = null ) {
		...
		// No need to apply trigger time to hours interval.
		if ( 'hours' !== $this->interval ) {
			$trigger_time = explode( ':', $this->trigger_time );
			$event_date->setTime( $trigger_time[0], $trigger_time[1] );
		}
