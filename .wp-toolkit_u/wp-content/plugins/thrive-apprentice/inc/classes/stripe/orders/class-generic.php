...
abstract class Generic {
	...
	public function __construct( $data, $status = TVA_Const::STATUS_COMPLETED ) {
		/**
		 * Check if tapp_product_id exists in the data.
		 * If it doesnt, that means the transaction is not from Thrive Apprentice.
		 */
		if ( ! Helper::check_array_key_recursive( $data, 'tapp_product_id' ) ) {
			return;
		}
	...
	public function save() {
		...
		$existing_canceled_orders = TVA_Order::get_orders_by_payment_id(  $this->data->id, TVA_Const::STATUS_FAILED );
		if ( ! empty( $existing_canceled_orders ) ) {
			$this->order->set_status( TVA_Const::STATUS_FAILED );
		} 
