	...
	public static function assessment_type( $attr = [], $content = '' ) {
		...
		if ( Main::$is_editor_page ) {
			...
			if ( in_array( static::$assessment_form_state, [ Main::STATE_AUTO, Main::STATE_SUBMIT ] ) ) {
				...
				$user_submissions = TVA_User_Assessment::get_user_submission( [ 'post_parent' => static::$assessment_from_request->ID ] );
				if ( static::$assessment_form_state === Main::STATE_AUTO && count( $user_submissions ) > 0 ) {
					/**
					 * If the form state is auto and the active user has form submissions the assessment type becomes result,
					 * but if latest assessment is not graded, the assessment type becomes confirmation.
					 */
					$assessment_type = Main::TYPE_RESULTS;
					if ( empty( $user_submissions[0]->get_grade() ) ) {
						$assessment_type = Main::TYPE_CONFIRMATION;
					}
				}
