...
class Main {
	...
	public function get_courses( $query = array(), $count = false ) {
		...
		add_filter( 'terms_clauses', [ $this, 'hidden_courses_logic' ], 10, 3 );
	...
	public function hidden_courses_logic( $clauses, $taxonomy, $args ) {
		if ( is_user_logged_in() && $taxonomy[0] === TVA_Const::COURSE_TAXONOMY ) {
			...
			// Include courses where the search keyword is present in the course name.
			$search_sql = '';
			if ( ! empty( $args['name__like'] ) ) {
				$search_keyword = esc_sql( $args['name__like'] );
				$search_sql = "t.name LIKE '%{$search_keyword}%' AND ";
			}

			// Label filtering for hidden and archived courses.
			$label_sql = '';
			if ( 
				isset( $args['meta_query'] ) && 
				isset( $args['meta_query']['tva_label'] ) &&
				isset( $args['meta_query']['tva_label'][0] ) &&
				! empty( $args['meta_query']['tva_label'][0]['value'] )
			) {
				$labels = $args['meta_query']['tva_label'][0]['value'];

				if ( is_array( $labels ) ) {
					$labels = implode( "','", array_map( 'esc_sql', $labels ) );
					$label_sql = "mt2.meta_key = 'tva_label' AND mt2.meta_value IN ('{$labels}') AND";
				}
			}

			$clauses['where'] .= " 
			OR ( mt1.meta_key = 'tva_status' AND mt1.meta_value = 'hidden' AND {$label_sql} {$search_sql} ({$learned_courses_sql} t.term_id IN (
			...
			OR ( mt1.meta_key = 'tva_status' AND mt1.meta_value = 'archived' AND {$label_sql} {$search_sql} ({$learned_courses_sql} t.term_id IN (
	...
	public function get_progress_percentage( $course ) {
		return tva_get_course_progress( $course );
	}
