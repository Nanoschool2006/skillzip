...
class Base {
	...
	public function remove_reminder( $datetime, $reminder, $event_args = [] ) {
		...
		$this->unschedule_event( $datetime, $event_args, static::EVENT_REMINDER );
	...
	public function unschedule_event( $date, $event_args = [], $event = null ) {
		...
		if ( $next_sched_stamp = wp_next_scheduled( $event, $this->get_event_args( $event_args ) ) ) {
			wp_unschedule_event( $next_sched_stamp, $event, $this->get_event_args( $event_args ) );
		}
	}
