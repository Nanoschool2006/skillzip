...
class Actions {
	...
	public static function add() {
		...
		// Hide quantity buttons if product has only 1 stock.
		add_action( 'woocommerce_before_add_to_cart_quantity', static function () {
			if ( self::product_has_one_stock() ) {
				echo '<div class="tcb-permanently-hidden">';

				// Nested to prevent calling product_has_one_stock() again.
				add_action( 'woocommerce_after_add_to_cart_quantity', static function () {
					echo '</div>';
				}, 20, 3 );
			}
		}, 20, 3 );

		// Add stock-1 class to cart items with 1 stock left.
		add_filter( 'woocommerce_cart_item_class', function( $class, $cart_item, $cart_item_key ) {
			if ( self::product_has_one_stock( $cart_item['data'] ) ) {
				$class .= ' stock-1';
			}
	...
	/**
	 * Check if product has only 1 stock.
	 *
	 * 
	 */
	public static function product_has_one_stock( $product = false ) {
		$product = ! $product ? wc_get_product() : $product;

		return $product && $product->get_manage_stock() && $product->get_stock_quantity() === 1;
	}
}
