...
function tve_ult_save_campaign( $model ) {
	if ( ! empty( $model['ID'] ) ) {
		...
		if ( ! empty( $model['copy'] ) ) {
			$model['post_title'] = __( 'Copy of ', 'thrive-ult' ) . $model['post_title'];
		}
	...
	if ( ! empty( $model['type'] ) ) {
		...
		if ( ! empty( $model['settings']['trigger']['ids'] ) ) {
			if ( function_exists( 'tve_flush_cache' ) ) {
				tve_flush_cache( $model['settings']['trigger']['ids'] );
			}
		}
	}
	...
	if ( isset( $model['lockdown'] ) ) {
		...
		if ( $model['lockdown'] == true ) {
			...
			/* When saving a campaign with promotion pages we need to flush the  cache on them */
			$promotion_pages = array_column( $model['lockdown_settings']['promotion'], 'id' );

			foreach ( $promotion_pages as $promotion_page_id ) {
				if ( function_exists( 'tve_flush_cache' ) ) {
					tve_flush_cache( $promotion_page_id );
				}
			}
		}
	}
