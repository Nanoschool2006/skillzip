...
class Thrive_Shortcodes {
	...
	public function init() {
		...
		add_filter( 'pre_do_shortcode_tag', function ( $output, $tag, $attr, $m ) {
			...
			/**
			 * Decide if a shortcode can be rendered inside the editor or not.
			 * By default, it should be not because at save it will be saved as static html and won't be displayed correctly on frontend after.
			 *
			 * @param {Boolean} $should_render_shortcode
			 * @param {String} $shortcode_tag
			 *
			 * @return {Boolean}
			 */
			$should_render_shortcode = apply_filters( 'thrive_theme_should_render_shortcode', Thrive_Utils::is_thrive_shortcode( $tag ) || ! TCB_Utils::in_editor_render( true ), $tag );

			if ( ! empty( $m[0] ) && ! $should_render_shortcode ) {
				$output = $m[0];
			}
	...
	public static function author_social_url( $attr = [], $content = '', $tag = '' ) {
		...
		if ( TCB_Utils::in_editor_render( true ) ) {
			...
			$resolved_key = self::resolve_social_key( $key, $links );
			$link         = empty( $links[ $resolved_key ] ) ? '#' : $links[ $resolved_key ];
		}
	...
	/**
	 * Resolve social network key with fallback support for X/Twitter
	 *
	 * For keys other than 't' or 'x', returns the key as-is.
	 * For 't' or 'x', returns the key if it exists in $links, otherwise falls back to the other ('x' or 't') if available.
	 * If neither 't' nor 'x' are present in $links, returns the original key.
	 * @param string $key The requested social network key
	 * @param array $links Available social links
	 * @return string The resolved key to use
	 */
	private static function resolve_social_key( $key, $links ) {
		// Early return for keys other than 't' or 'x'
		if ( ! in_array( $key, [ 't', 'x' ], true ) ) {
			return $key;
		}

		// If requested key exists, use it
		if ( ! empty( $links[ $key ] ) ) {
			return $key;
		}

		// Fallback: use the other X/Twitter key if available
		$fallback_key = $key === 't' ? 'x' : 't';
		if ( ! empty( $links[ $fallback_key ] ) ) {
			return $fallback_key;
		}
