...
class TVA_Assessment extends TVA_Post {
	...
	public function is_completed( $user_id = 0 ) {
		...
		$completion_credit_type = Grading_Base::get_assessment_completion_credit( $this->ID );
		switch ( $completion_credit_type ) {
			case 'upon_passing':
				$meta_key = TVA_User_Assessment::ASSESSMENT_PASSED;
				break;
			case 'upon_grading':
				$meta_key = TVA_User_Assessment::ASSESSMENT_GRADED;
				break;
			default:
				$meta_key = TVA_User_Assessment::SUBMISSION_COUNTER_META;
				break;
		}

		$counter = (int) get_user_meta( $user_id, $meta_key . $this->ID, true ) ?: 0;

		// Increase counter if user has completed the course.
		$course = $this->get_course_v2();
		$customer = new TVA_Customer( $user_id );
		if ( $customer::get_user_completed_meta( $user_id, $course->get_id() ) ) {
			$counter = 1;
		}
