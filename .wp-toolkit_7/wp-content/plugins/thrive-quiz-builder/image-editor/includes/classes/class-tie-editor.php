...
class TIE_Editor {
	...
	public function print_backbone_templates() {
		$templates = tve_dash_get_backbone_templates( tqb()->plugin_path() . 'thrive-dashboard/templates/modal', 'templates' );
	...
	public function add_scripts() {
		...
		tie_enqueue_script( 'tie-modal', TVE_DASH_URL . '/js/dist/modal' . $js_suffix, [
			'jquery',
			'backbone',
		], '', true );
		tie_enqueue_style( 'tie-dash-modal-css', TVE_DASH_URL . '/css/modal.css' );
	}
	...
	/**
	 * Get Google Fonts API key from endpoint with transient caching
	 *
	 * @return string API key or empty string on error
	 */
	private function get_google_fonts_api_key() {
		// Check transient first
		if ( false !== $key = get_transient( 'tie_google_fonts_api_key' ) ) {
			return $key;
		}

		$endpoint = 'https://thrivethemesapi.com/api/secrets/v1/api_key_google_fonts';
		...
		$key = sanitize_text_field( $data['data']['value'] );
		
		// Cache for 24 hours
		set_transient( 'tie_google_fonts_api_key', $key, 24 * HOUR_IN_SECONDS );
	...
	public function get_fonts() {
		...
		$key      = $this->get_google_fonts_api_key();
