...
class TCB_Post_List_Shortcodes {
	...
	public function author_link_shortcode( $attr, $content, $tag ) {
		...
		if ( TCB_Utils::in_editor_render( true ) ) {
			...
			/* Fallback for Twitter/X: check both 't' and 'x' keys since URL may be stored with either */
			if ( empty( $link ) && in_array( $key, [ 't', 'x' ], true ) ) {
				$fallback_key = $key === 't' ? 'x' : 't';
				$link         = empty( $links[ $fallback_key ] ) ? '' : $links[ $fallback_key ];
			}
		}
