...
class TQB_Database {
	...
	/**
	 * Request-level cache for quiz user lookups
	 * @var array
	 */
	...
	public function get_report_data_count_event_type( $filter ) {
		...
		if ( ! empty( $filter['conversion_goal'] ) ) {
			// Only apply conversion goal filter to conversion events (event_type = 2)
			// Impressions (event_type = 1) should not be filtered by optin/social_share
			if ( $filter['conversion_goal'] === Thrive_Quiz_Builder::CONVERSION_GOAL_SOCIAL ) {
				$sql .= 'AND (`event_type` = 1 OR (`event_type` = 2 AND `social_share` = 1)) ';
			} else {
				$sql .= 'AND (`event_type` = 1 OR (`event_type` = 2 AND `optin` = 1)) ';
			}
		}
		...
		if ( ! empty( $filter['start_date'] ) && ! empty( $filter['end_date'] ) ) {
			...
			$params [] = date( 'Y-m-d H:i:s', ( strtotime( '+1 day', strtotime( $filter['end_date'] ) ) + $timezone_diff ) );
		}
	...
	public function create_event_log_entry( $model ) {
		...
		if ( ! empty( $event_log ) ) {
			...
			// Clear statistics cache when event is updated (social shares, subscribers)
			if ( ! empty( $data['page_id'] ) ) {
				$page = get_post( $data['page_id'] );
				if ( $page && ! empty( $page->post_parent ) ) {
					TQB_Quiz_Manager::clear_quiz_statistics_cache( $page->post_parent );
				}
			}
		...
		$result = $this->wpdb->insert( tqb_table_name( 'event_log' ), $data );

		// Clear statistics cache when new event is created (social shares, subscribers)
		if ( $result && ! empty( $data['page_id'] ) ) {
			$page = get_post( $data['page_id'] );
			if ( $page && ! empty( $page->post_parent ) ) {
				TQB_Quiz_Manager::clear_quiz_statistics_cache( $page->post_parent );
			}
	...
	public function get_quiz_user( $unique, $quiz_id ) {
		...
		$cache_key = md5( $unique . '_' . $quiz_id );

		// Check request-level cache first
		if ( isset( self::$user_cache[ $cache_key ] ) ) {
		...
		$user = $this->wpdb->get_row( $this->prepare( $sql, $params ), ARRAY_A );

		// Cache result (even if null/empty)
	...
	public function save_quiz_user( $model = array() ) {
		...
		// If stats tracking is disabled and this is a completion update, skip it.
		if ( ! tqb_is_stats_tracking_enabled() && ! empty( $data['completed_quiz'] ) && ! empty( $data['id'] ) ) {
			return $data['id'];
		}
		...
		if ( ! empty( $data['id'] ) ) {
			...
			if ( $update_rows !== false ) {
				// Clear statistics cache when user completes quiz
				if ( ! empty( $data['quiz_id'] ) ) {
					TQB_Quiz_Manager::clear_quiz_statistics_cache( $data['quiz_id'] );
				}
				// Clear cache for this user (if we have the unique identifier)
				if ( ! empty( $data['random_identifier'] ) && ! empty( $data['quiz_id'] ) ) {
					$cache_key = md5( $data['random_identifier'] . '_' . $data['quiz_id'] );
					unset( self::$user_cache[ $cache_key ] );
				}
				return $data['id'];
			}
		...
		// Clear statistics cache when new user starts quiz
		if ( ! empty( $data['quiz_id'] ) ) {
			TQB_Quiz_Manager::clear_quiz_statistics_cache( $data['quiz_id'] );
		}
		// Cache the newly created user without triggering an extra DB query
		if ( ! empty( $data['random_identifier'] ) && ! empty( $data['quiz_id'] ) && $user_id ) {
			$cache_key       = md5( $data['random_identifier'] . '_' . $data['quiz_id'] );
			$user            = $data;
			$user['id']      = $user_id;
			self::$user_cache[ $cache_key ] = $user;
	...
	public function save_user_answer( $model = array() ) {
		...
		// If stats tracking is disabled, skip saving answers (but return success for quiz to continue).
		if ( ! tqb_is_stats_tracking_enabled() ) {
			return ! empty( $data['id'] ) ? $data['id'] : 1;
		}
	...
	/**
	 * Get existing user answers in bulk for duplicate checking
	 * Returns array of answer_ids that already exist for the given user and quiz
	 *
	 * @param int   $quiz_id
	 * @param int   $user_id
	 * @param array $answer_ids Array of answer IDs to check
	 *
	 * @return array Array of existing answer_ids
	 */
	public function get_user_answers_bulk( $quiz_id, $user_id, $answer_ids ) {
		if ( empty( $quiz_id ) || empty( $user_id ) || empty( $answer_ids ) || ! is_array( $answer_ids ) ) {
			return array();
		}

		// Sanitize answer_ids to integers
		...
		$data         = array_merge( array( $quiz_id, $user_id ), $answer_ids );

		$sql = 'SELECT answer_id FROM ' . tqb_table_name( 'user_answers' ) . ' WHERE quiz_id = %d AND user_id = %d AND answer_id IN (' . $placeholders . ')';
		$sql = $this->prepare( $sql, $data );
	...
	public function calculate_user_points( $user_unique, $quiz_id ) {
		...
		if ( is_array( $data ) && ! empty( $data ) ) {
			$end_result['category_breakdown'] = array_values( array_filter( $data, function ( $item ) {
				return isset( $item['result_id'] ) && 0 !== (int) $item['result_id'];
			} ) );
		}
		...
		if ( empty( $data ) && Thrive_Quiz_Builder::QUIZ_TYPE_PERSONALITY == $quiz_type['type'] ) {
			...
			$category_breakdown = $end_result['category_breakdown'];
			foreach ( $data as $result ) {
				if ( empty( $end_result['user_points'] ) || $result['user_points'] > $end_result['user_points'] ) {
					$end_result = $result;
				}
			}
			$end_result['category_breakdown'] = $category_breakdown;
		}
	...
	public function get_quiz_map( $quiz_id ) {
		// Cache key for this quiz's map
		$cache_key = 'tqb_quiz_map_' . $quiz_id;
		...
		$result = $this->wpdb->get_results( $this->prepare( $sql, $params ), ARRAY_A );
	...
	/**
	 * Invalidate quiz map cache for a specific quiz
	 *
	 * @param int $quiz_id
	 *
	 * @return bool
	 */
	public function invalidate_quiz_map_cache( $quiz_id ) {
		$cache_key = 'tqb_quiz_map_' . $quiz_id;
		return delete_transient( $cache_key );
	}

	/**
	 * Get category breakdown with names for a quiz
	 *
	 * Enriches category_breakdown data with category names from the results table.
	 * Returns ALL categories for the quiz, with 0 points for unscored ones.
	 *
	 * @param array $category_breakdown Raw category breakdown from user quiz data.
	 * @param int   $quiz_id            Quiz ID.
	 *
	 * @return array Enriched category data with id, name, and points.
	 */
	public function get_category_breakdown_with_names( $category_breakdown, $quiz_id ) {

		if ( empty( $quiz_id ) || ! isset( $this->wpdb ) || ! is_object( $this->wpdb ) ) {
			return array();
		}

		$quiz_id = (int) $quiz_id;

		// Get ALL categories for this quiz
		$all_categories = $this->get_quiz_results( $quiz_id );

		if ( empty( $all_categories ) ) {
			return array();
		}

		// Index user scores by result_id for quick lookup
		$scores_by_id = array();

		if ( ! is_array( $category_breakdown ) ) {
			$category_breakdown = array();
		}

		foreach ( $category_breakdown as $category ) {
			if ( ! is_array( $category ) || ! isset( $category['result_id'] ) ) {
				continue;
			}
			$scores_by_id[ (int) $category['result_id'] ] = isset( $category['user_points'] ) ? (int) $category['user_points'] : 0;
		}
		...
		foreach ( $all_categories as $category ) {
			...
			$result[] = array(
				'id'     => $cat_id,
				'name'   => isset( $category['text'] ) ? $category['text'] : '',
				'points' => isset( $scores_by_id[ $cat_id ] ) ? $scores_by_id[ $cat_id ] : 0,
			);
		}
	...
	/**
	 * Calculate share and percent values for categories
	 *
	 * @param array $categories   Array of categories with 'points' key.
	 * @param array $max_possible Optional. Array of max possible points keyed by category ID.
	 *
	 * @return array Categories with added 'share' and optionally 'percent' keys.
	 */
	public function calculate_category_shares( $categories, $max_possible = array() ) {
		...
		// First pass: validate and calculate total points
		foreach ( $categories as $category ) {
			if ( ! is_array( $category ) || ! isset( $category['points'] ) ) {
				continue;
			}
			$total_points += (int) $category['points'];
		}

		// Second pass: add share and percent calculations
		foreach ( $categories as $category ) {
			if ( ! is_array( $category ) || ! isset( $category['points'] ) ) {
				continue;
			}
			...
			if ( $total_points > 0 ) {
				$share             = ( $points / $total_points ) * 100;
				// Format: whole numbers >= 1%, 1 decimal < 1%
				$category['share'] = $share >= 1 ? round( $share ) : round( $share, 1 );
			}

			// Calculate percent against maximum possible if provided
			if ( empty( $max_possible ) || ! is_array( $max_possible ) ) {
				$result[] = $category;
				continue;
			}

			$cat_id = isset( $category['id'] ) ? (int) $category['id'] : 0;
			$max    = isset( $max_possible[ $cat_id ] ) ? (int) $max_possible[ $cat_id ] : 0;

			$category['percent'] = 0;

			if ( $max > 0 ) {
				$percent             = ( $points / $max ) * 100;
				$category['percent'] = $percent >= 1 ? round( $percent ) : round( $percent, 1 );
			}
	...
	/**
	 * Get maximum possible points per category for a quiz
	 *
	 * For each question, finds the highest-scoring answer per category,
	 * then sums those maximums across all questions.
	 *
	 * @param int $quiz_id Quiz ID.
	 *
	 * @return array Associative array of result_id => max_possible_points.
	 */
	public function get_category_max_possible_points( $quiz_id ) {
		...
		// Get all answers with their category and points
		$sql = 'SELECT
					a.question_id,
					a.result_id,
					a.points
				FROM ' . tge_table_name( 'answers' ) . ' a
				INNER JOIN ' . tge_table_name( 'questions' ) . ' q ON a.question_id = q.id
				WHERE q.quiz_id = %d';

		$answers = $this->wpdb->get_results( $this->prepare( $sql, array( $quiz_id ) ), ARRAY_A );

		if ( empty( $answers ) || ! is_array( $answers ) ) {
			return array();
		}

		// For each question, find max points per category
		$question_max = array();
		foreach ( $answers as $answer ) {
			// Validate required array keys exist
			if ( ! isset( $answer['question_id'], $answer['result_id'], $answer['points'] ) ) {
				continue;
			}

			$q_id = (int) $answer['question_id'];
			$r_id = (int) $answer['result_id'];
			$pts  = (int) $answer['points'];

			if ( ! isset( $question_max[ $q_id ] ) ) {
				$question_max[ $q_id ] = array();
			}
			if ( ! isset( $question_max[ $q_id ][ $r_id ] ) || $pts > $question_max[ $q_id ][ $r_id ] ) {
				$question_max[ $q_id ][ $r_id ] = $pts;
			}
		}

		// Sum max points per category across all questions
		$max_possible = array();
		foreach ( $question_max as $q_id => $categories ) {
			foreach ( $categories as $r_id => $max_pts ) {
				if ( ! isset( $max_possible[ $r_id ] ) ) {
					$max_possible[ $r_id ] = 0;
				}
				$max_possible[ $r_id ] += $max_pts;
			}
		}
	...
	/**
	 * Get user stats (users_started and completed_count) for multiple quizzes in a single query
	 *
	 * @param array $quiz_ids Array of quiz IDs
	 *
	 * @return array Associative array keyed by quiz_id: [ quiz_id => ['users_started' => X, 'completed_count' => Y] ]
	 */
	public function get_batch_quiz_user_stats( $quiz_ids ) {
		if ( empty( $quiz_ids ) ) {
			return array();
		}
		...
		$sql          = $this->wpdb->prepare(
			'SELECT quiz_id, COUNT(*) as users_started, SUM(CASE WHEN completed_quiz = 1 THEN 1 ELSE 0 END) as completed_count FROM ' . tqb_table_name( 'users' ) . " WHERE quiz_id IN ($placeholders) AND ignore_user IS NULL GROUP BY quiz_id",
			$quiz_ids
		);
		...
		foreach ( $results as $row ) {
			$stats[ $row->quiz_id ] = array(
				'users_started'   => (int) $row->users_started,
	...
	/**
	 * Get social share counts for multiple page IDs in a single query
	 *
	 * @param array $page_ids Array of page IDs
	 *
	 * @return array Associative array keyed by page_id: [ page_id => count ]
	 */
	public function get_batch_social_shares_counts( $page_ids ) {
		if ( empty( $page_ids ) ) {
			return array();
		}
		...
		$sql          = $this->wpdb->prepare(
			'SELECT page_id, COUNT(*) as count FROM ' . tqb_table_name( 'event_log' ) . " WHERE page_id IN ($placeholders) AND social_share = 1 GROUP BY page_id",
			$page_ids
		);
		...
		foreach ( $results as $row ) {
			$counts[ $row->page_id ] = (int) $row->count;
		}
	...
	/**
	 * Get subscriber counts for multiple page IDs in a single query
	 *
	 * @param array $page_ids Array of page IDs
	 *
	 * @return array Associative array keyed by page_id: [ page_id => count ]
	 */
	public function get_batch_page_subscribers( $page_ids ) {
		if ( empty( $page_ids ) ) {
			return array();
		}
		...
		$placeholders = implode( ',', array_fill( 0, count( $page_ids ), '%d' ) );
		$sql          = $this->wpdb->prepare(
			'SELECT page_id, COUNT(*) as count FROM ' . tqb_table_name( 'event_log' ) . " WHERE page_id IN ($placeholders) AND event_type = 2 AND optin = 1 GROUP BY page_id",
			$page_ids
		);
