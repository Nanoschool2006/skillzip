...
class TQB_Quiz_Manager {
	 ...
	 * @param boolean $include_statistics Whether to include statistics (started, finished, subscribers, social shares)
	 *
	 * @return array $posts
	 */
	public static function get_quizzes( $filters = array(), $include_details = true, $include_statistics = true ) {
		...
		if ( $include_details ) {
			foreach ( $posts as $index => $post ) {
				$post            = TQB_Quiz_Manager::get_quiz_post_details( $post, $include_statistics );
				$posts[ $index ] = $post;
			}
		}
	 ...
	 * @return array
	 */
	public static function get_searched_quizzes( $search_word = '', $include_statistics = true ) {
		$defaults        = array(
			'posts_per_page' => - 1,
			'post_type'      => TQB_Post_types::QUIZ_POST_TYPE,
			'orderby'        => 'post_date',
	 ...
	 * @return array
	 */
	public static function get_specific_quizzes( $quizzes_id = array(), $include_statistics = true ) {
		$posts = array();

		foreach ( $quizzes_id as $quiz_id ) {
			$post    = get_post( $quiz_id );
			$posts[] = TQB_Quiz_Manager::get_quiz_post_details( $post, $include_statistics );
		}
	...
	/**
	 * @param WP_Post $post
	 * @param bool $include_statistics Whether to include statistics (started, finished, subscribers, social shares)
	 *
	 * @return mixed
	 */
	public static function get_quiz_post_details( $post, $include_statistics = true ) {
		...
		if ( $include_statistics ) {
			...
			$post->completed_count = null;
			$post->users_started   = null;
			$post->social_shares   = null;
	...
	public function delete_quiz( $force_delete = true ) {
		...
		if ( $force_delete ) {
			...
			self::clear_quiz_statistics_cache( $this->quiz->ID );
...
//					$badge_url = $badge->get_url();
					if ( ! empty( $badge_url ) ) {
						...
						if ( ! empty( $image_post[0] ) ) {
							...
							$shortcode_content['page']['fonts'][] = '//fonts.bunny.net/css?family=Roboto'; //default font for BE
	 ...
	 * @param int    $user_id    optional user ID to skip user lookup
	 *
	 * @return bool
	 */
	public static function register_answer( $answer_id, $user_unique, $quiz_id, $answer_text = '', $user_id = null ) {
		...
		// get user check if existing (skip if user_id provided)
		global $tqbdb;
		if ( null !== $user_id ) {
			// User ID provided, validate it exists and get quiz_id
			$user = $tqbdb->get_users( array( 'id' => $user_id, 'quiz_id' => $quiz_id ), ARRAY_A );
	...
	/**
	 * Register multiple quiz question answers in bulk
	 * Optimized version that processes multiple answers with minimal database queries
	 *
	 * @param array  $answer_ids  Array of answer IDs to register
	 * @param string $user_unique User unique identifier
	 * @param int    $quiz_id    Quiz ID
	 * @param string $answer_text Text provided by user for Open Ended Questions (applies to all answers)
	 * @param int    $user_id    Optional user ID to skip user lookup
	 *
	 * @return array Array of successfully registered answer IDs
	 */
	public static function register_answers_bulk( $answer_ids, $user_unique, $quiz_id, $answer_text = '', $user_id = null ) {
		if ( empty( $answer_ids ) || ! is_array( $answer_ids ) || empty( $quiz_id ) ) {
			return array();
		}
		...
		// Validate user once
		if ( null !== $user_id ) {
			// User ID provided, validate it exists
			$user = $tqbdb->get_users( array( 'id' => $user_id, 'quiz_id' => $quiz_id ), ARRAY_A );
			if ( empty( $user ) || ! is_array( $user ) ) {
				return array();
			}
		...
		// Fetch all answers in one query using custom SQL (since get_answers doesn't support multiple IDs)
		global $wpdb;
		$placeholders = implode( ',', array_fill( 0, count( $answer_ids ), '%d' ) );
		$data         = array_merge( array( $quiz_id ), $answer_ids );
		$sql          = 'SELECT * FROM ' . tge_table_name( 'answers' ) . ' WHERE quiz_id = %d AND id IN (' . $placeholders . ') ORDER BY `order` ASC';
		...
		// Process image fields if needed
		...
		// Batch check for existing answers
		$existing_answer_ids = $tqbdb->get_user_answers_bulk( $quiz_id, $user_id, $answer_ids );

		// Prepare insert data
		$question_manager = new TGE_Question_Manager( $quiz_id );
		$registered_ids   = array();

		// Batch fetch all unique questions to avoid N+1 queries
		$question_ids = array_unique( array_column( $answers, 'question_id' ) );
		$questions_map = array();
		if ( ! empty( $question_ids ) ) {
			$placeholders = implode( ',', array_fill( 0, count( $question_ids ), '%d' ) );
			$data         = array_merge( array( $quiz_id ), $question_ids );
			$sql          = 'SELECT * FROM ' . tge_table_name( 'questions' ) . ' WHERE quiz_id = %d AND id IN (' . $placeholders . ')';
			$sql          = $wpdb->prepare( $sql, $data );
			$all_questions = $wpdb->get_results( $sql, ARRAY_A );

			// Process image fields and create lookup map
			foreach ( $all_questions as $q ) {
				if ( ! empty( $q['image'] ) ) {
					$q['image'] = json_decode( $q['image'] ) ? json_decode( $q['image'] ) : $q['image'];
				}
				$questions_map[ (int) $q['id'] ] = $q;
			}
		}

		// Collect answers to insert and fire hooks
		$answers_to_insert = array();
		$has_answer_text   = ! empty( $answer_text );

		foreach ( $answers as $answer ) {
			$answer_id = (int) $answer['id'];

			// Skip if already exists
			if ( in_array( $answer_id, $existing_answer_ids, true ) ) {
				continue;
			}
			...
			// Get question data for the hook (from batch-fetched map)
			$question = isset( $questions_map[ (int) $answer['question_id'] ] ) ? $questions_map[ (int) $answer['question_id'] ] : null;
			 ...
			 * @api
			 */
			do_action( 'thrive_quizbuilder_answer_submitted', TQB_Quiz_Manager::get_quiz_details( $quiz_id, $user_unique, '', '', $user_id ), array(
				'quiz_id'            => $answer['quiz_id'],
				'question_id'        => $answer['question_id'],
				'question_answer'    => empty( $answer_text ) ? $answer['text'] : $answer_text,
				'question_answer_id' => $answer['id'],
			...
			$answers_to_insert[] = $user_answer;
		}

		// When multiple answers are present, insert each one individually to preserve existing behavior
		if ( count( $answers_to_insert ) > 1 ) {
			// Use individual inserts in a loop to match save_user_answer behavior for each answer
			// This ensures answer_text and any related side effects are handled consistently
			foreach ( $answers_to_insert as $user_answer ) {
				$inserted_id = $tqbdb->save_user_answer( $user_answer );
				if ( $inserted_id ) {
					$registered_ids[] = (int) $user_answer['answer_id'];
				}
			}
		} elseif ( count( $answers_to_insert ) === 1 ) {
			// Single insert
			$inserted_id = $tqbdb->save_user_answer( $answers_to_insert[0] );
			if ( $inserted_id ) {
				$registered_ids[] = (int) $answers_to_insert[0]['answer_id'];
			}
		}
	 ...
	 * @return array
	 */
	public static function get_quiz_details( $quiz_id, $user_unique, $result = '', $email = '', $user_id = null ) {
		$answers = array();
	...
	public static function get_quiz_user( $user_unique = null, $quiz_id = null, $full_data = false, $filters = array() ) {
		...
		// If stats tracking is disabled, mark user as ignored so they don't appear in reports
		if ( ! tqb_is_stats_tracking_enabled() ) {
			$ignore_user = 1;
		}
	...
	/**
	 * Get statistics for multiple quizzes at once
	 *
	 * @param array $quiz_ids Array of quiz IDs
	 *
	 * @return array Array of statistics keyed by quiz ID
	 */
	public static function get_quizzes_statistics( $quiz_ids = array() ) {
		global $tqbdb;

		$statistics    = array();
		$uncached_ids  = array();
		$cache_expires = 5 * MINUTE_IN_SECONDS; // Cache for 5 minutes

		// 1. Check cache first, collect uncached IDs
		foreach ( $quiz_ids as $quiz_id ) {
			$quiz_id      = intval( $quiz_id );
			$cache_key    = 'tqb_quiz_stats_' . $quiz_id;
			$cached_stats = get_transient( $cache_key );

			if ( false !== $cached_stats ) {
				$statistics[ $quiz_id ] = $cached_stats;
			} else {
				$uncached_ids[] = $quiz_id;
			}
		}

		// 2. Early return if all are cached
		if ( empty( $uncached_ids ) ) {
			return $statistics;
		}

		// 3. Batch user stats — one query for users_started + completed_count
		$batch_user_stats = $tqbdb->get_batch_quiz_user_stats( $uncached_ids );

		// 4. Batch results pages — one get_posts() call for all quizzes
		$results_pages     = get_posts( array(
			'post_parent__in' => $uncached_ids,
			'post_type'       => Thrive_Quiz_Builder::QUIZ_STRUCTURE_ITEM_RESULTS,
			'posts_per_page'  => -1,
			'fields'          => 'all',
		) );
		$results_page_map  = array(); // quiz_id => results_page_id
		$results_page_ids  = array();
		foreach ( $results_pages as $page ) {
			$results_page_map[ $page->post_parent ] = $page->ID;
			$results_page_ids[]                     = $page->ID;
		}

		// 5. Batch social shares — one query for all result pages
		$batch_social_shares = $tqbdb->get_batch_social_shares_counts( $results_page_ids );

		// 6. Batch structure meta — prime WP's meta cache in one query
		update_postmeta_cache( $uncached_ids );

		// Collect all page IDs needed for subscriber counts
		$all_subscriber_page_ids = array();
		$quiz_subscriber_pages   = array(); // quiz_id => ['optin' => id, 'results' => id]

		foreach ( $uncached_ids as $quiz_id ) {
			$structure_data = get_post_meta( $quiz_id, TQB_Post_meta::META_NAME_FOR_QUIZ_STRUCTURE, true );
			$pages          = array();

			if ( ! empty( $structure_data ) && is_numeric( $structure_data['optin'] ) ) {
				$pages['optin']            = (int) $structure_data['optin'];
				$all_subscriber_page_ids[] = (int) $structure_data['optin'];
			}

			if ( ! empty( $structure_data ) && is_numeric( $structure_data['results'] ) ) {
				$pages['results']          = (int) $structure_data['results'];
				$all_subscriber_page_ids[] = (int) $structure_data['results'];
			}

			$quiz_subscriber_pages[ $quiz_id ] = $pages;
		}

		// 7. Batch subscribers — one query for all optin + results page IDs
		$all_subscriber_page_ids = array_unique( $all_subscriber_page_ids );
		$batch_subscribers       = $tqbdb->get_batch_page_subscribers( $all_subscriber_page_ids );

		// 8. Assemble stats per quiz and cache
		foreach ( $uncached_ids as $quiz_id ) {
			$user_stats = isset( $batch_user_stats[ $quiz_id ] ) ? $batch_user_stats[ $quiz_id ] : array(
				'users_started'   => 0,
				'completed_count' => 0,
			);

			$results_page_id = isset( $results_page_map[ $quiz_id ] ) ? $results_page_map[ $quiz_id ] : 0;
			$social_shares   = $results_page_id && isset( $batch_social_shares[ $results_page_id ] ) ? $batch_social_shares[ $results_page_id ] : 0;

			$subscribers    = 0;
			$subscriber_pgs = $quiz_subscriber_pages[ $quiz_id ];
			if ( ! empty( $subscriber_pgs['optin'] ) && isset( $batch_subscribers[ $subscriber_pgs['optin'] ] ) ) {
				$subscribers += $batch_subscribers[ $subscriber_pgs['optin'] ];
			}
			if ( ! empty( $subscriber_pgs['results'] ) && isset( $batch_subscribers[ $subscriber_pgs['results'] ] ) ) {
				$subscribers += $batch_subscribers[ $subscriber_pgs['results'] ];
			}

			$stats = array(
				'completed_count' => $user_stats['completed_count'],
				'users_started'   => $user_stats['users_started'],
				'social_shares'   => $social_shares,
				'subscribers'     => $subscribers,
			);

			set_transient( 'tqb_quiz_stats_' . $quiz_id, $stats, $cache_expires );
			$statistics[ $quiz_id ] = $stats;
		}
	...
	/**
	 * Clear statistics cache for a specific quiz
	 *
	 * @param int $quiz_id Quiz ID
	 */
	public static function clear_quiz_statistics_cache( $quiz_id ) {
		delete_transient( 'tqb_quiz_stats_' . intval( $quiz_id ) );
	}

	/**
	 * Clear statistics cache for all quizzes
	 */
	public static function clear_all_quiz_statistics_cache() {
		global $wpdb;

		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
				$wpdb->esc_like( '_transient_tqb_quiz_stats_' ) . '%'
			)
		);

		// Also clear transient timeout options
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
				$wpdb->esc_like( '_transient_timeout_tqb_quiz_stats_' ) . '%'
			)
		);
	}
	...
	public function delete_quiz_results_and_user_answers( $quiz_id = 0 ) {
		...
		// Clear statistics cache after deleting quiz data
		self::clear_quiz_statistics_cache( $quiz_id );
	...
	public function reset_stats() {
		...
		/**
		 * Clear statistics cache after reset
		 */
		if ( $reset_stats ) {
			self::clear_quiz_statistics_cache( $this->quiz->ID );
		}
	 ...
	 * @param int      $quiz_id
	 * @param int      $user_id
	 * @param int|null $limit Optional limit for number of answers to return (null = all answers)
	 *
	 * @return array
	 */
	public static function get_user_answers_in_order( $quiz_id, $user_id, $limit = null ) {
		global $tqbdb;

		// If limit is set, we still fetch all answers but will break early
		// For better performance with large datasets, we could modify the query, but ordering logic requires all answers
		...
		if ( ! empty( $quiz_map ) && ! empty( $quiz_map[0] ) && $quiz_map[0]['start'] === '1' ) {
			...
			while ( ! empty( $question_answers ) ) {
				...
				if ( null !== $limit && count( $answers ) >= $limit ) {
					break;
				}
		...
		// Apply limit to final array if not already applied during processing
		if ( null !== $limit && count( $answers ) > $limit ) {
