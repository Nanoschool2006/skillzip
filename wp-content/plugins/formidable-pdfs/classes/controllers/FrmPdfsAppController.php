...
class FrmPdfsAppController {
	...
	private static function generate_pdf_html( $entry, $args ) {
		...
		// Add the CSS.
		return str_replace( $css_placeholder, self::get_pdf_css( $entry, $args ), $html );
	}
	...
	private static function replace_internal_and_external_images( &$html ) {
		...
		foreach ( $matches as $match ) {
			$img         = $match[0];
			$img_url     = $match['url'];
			$is_internal = $valid_upload_dir && str_starts_with( $img_url, $upload_dir['baseurl'] );
	...
	private static function contains_full_width_columns_only( $element ) {
		...
		foreach ( $element->childNodes as $child_node ) {
			...
			if ( preg_match( '/frm\d{1,2}/', $class ) && ! str_contains( $class, 'frm12' ) ) {
				return false;
			}
		}
	...
	private static function get_default_entry_pdf_args() {
		...
		return array(
			'mode'           => $mode,
			// Comma separated IDs of exclude fields.
			'exclude_fields' => '',
			'include_extras' => '',
		);
	}
	...
	private static function get_fields_for_export( $entry, $args ) {
		 ...
		 * }
		 */
		return apply_filters( 'frm_pdfs_fields_for_export', $fields, compact( 'entry' ) );
	}
	...
	private static function remove_invisible_fields( $args, &$fields ) {
		$include_invisible = ! empty( $args['include_extras'] ) && str_contains( $args['include_extras'], 'admin_only' );
	...
	private static function get_entry_pdf_file_name( $entry, $args ) {
		...
		if ( $entry && str_contains( $file_name, '[' ) ) {
