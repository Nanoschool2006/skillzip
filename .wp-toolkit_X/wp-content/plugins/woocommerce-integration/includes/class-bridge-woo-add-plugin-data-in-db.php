...
if ( ! class_exists( 'BridgeWooAddPluginDataInDB' ) ) {
	/**
	 * This class adds the plugin data in database
	 */
	class BridgeWooAddPluginDataInDB {
		 ...
		 * Store URL
		 *
		 * @var string  Stores the URL of store. Retrieves updates from this store
		 */
		private $storeUrl = '';
		...
		public function licenseInformation( $licensing_info ) {
			...
			$license_status_text = $this->displayLicenseStatus( $status, @$display );
			...
			if ( false !== $status && 'valid' === $status ) {
				$license_action = '<input type = "submit" class = "button-primary" name = "edd_' . $this->pluginSlug . '_license_deactivate" value = "' . __( 'Deactivate License', 'woocommerce-integration' ) . '"/>';
			} elseif ( 'expired' === $status && ( ! empty( $display ) || '' != $display ) ) {
				$license_action  = '<input type="submit" class="button-primary" name="edd_' . $this->pluginSlug . '_license_activate" value="' . __( 'Activate License', 'woocommerce-integration' ) . '"/>';
				$license_action .= '<input type="button" class="button-primary" name="edd_' . $this->pluginSlug . '_license_renew" value="' . __( 'Renew License', 'woocommerce-integration' ) . '" onclick="window.open( \'' . $renew_link . '\' )"/>';
		...
		public function licenseMessages( $eb_licensing_messages ) {
			...
			if ( isset( $_POST['edd_' . $this->pluginSlug . '_license_key'] ) ) {
				...
				if ( isset( $_POST['edd_' . $this->pluginSlug . '_license_key'] ) &&
						empty( $_POST['edd_' . $this->pluginSlug . '_license_key'] ) ) {
					//If empty, show error message
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'Please enter license key for %s', 'woocommerce-integration' ), $this->pluginName ),
						'error'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						__( 'No response from server. Please try again later.', 'woocommerce-integration' ),
						'error'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'License key is not valid. Please check your license key and try again', 'woocommerce-integration' ), $this->pluginName ),
						'error'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'License key for %s is activated.', 'woocommerce-integration' ), $this->pluginName ),
						'updated'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'License key for %s have been Expired. Please, Renew it'.  $display, 'woocommerce-integration' ), $this->pluginName ),
						'error'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'Please enter valid license key for %s', 'woocommerce-integration' ), $this->pluginName ),
						'error'
					);
					...
					add_settings_error(
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'License key for %s is Disabled.', 'woocommerce-integration' ), $this->pluginName ),
						'error'
					);
					...
					add_settings_error( 
						'wdm_' . $this->pluginSlug . '_errors',
						esc_attr( 'settings_updated' ),
						sprintf( __( 'License Key for %s is already activated at : %s', 'woocommerce-integration' ), $this->pluginName, $display ),
						'error'
					);
				} elseif ( $status !== false ) {
...
//							}
		...
		public function setTransientOnActivation( $licenseStatus )
		{
			...
			if ( isset( $transVar ) ) {
				...
				if ( ! empty( $licenseStatus ) ) {
					include_once plugin_dir_path(  __FILE__  ) . 'class-bridge-woo-get-plugin-data.php';
					BridgeWooGetPluginData::setResponseData(  $licenseStatus, '', $this->pluginSlug, true  );
				}
			}
		}
		...
		public function invalidStatusMessages( $status, $display )
		{
			if ( 'invalid' == $status && ( !empty( $display ) || '' != $display ) ) { //Invalid license key   and site
				add_settings_error( 
					'wdm_' . $this->pluginSlug . '_errors',
					esc_attr( 'settings_updated' ),
					sprintf( __( 'License Key for %s is already activated at : ' . $display, 'woocommerce-integration' ), $this->pluginName ),
					'error'
				 );
				...
				add_settings_error( 
					'wdm_' . $this->pluginSlug . '_errors',
					esc_attr( 'settings_updated' ),
					__( 'Site inactive( Press Activate license to activate plugin )', 'woocommerce-integration' ),
					'error'
				 );
				...
				add_settings_error( 
					'wdm_' . $this->pluginSlug . '_errors',
					esc_attr( 'settings_updated' ),
					sprintf( __( 'License Key for %s is deactivated', 'woocommerce-integration' ), $this->pluginName ),
					'updated'
				 );
			}
		}
		...
		public function displayLicenseStatus( $status, $display )
		{
			if ( $status !== false && $status == 'valid' ) {
				$license_status_text = '<span style="color:green;">' . __( 'Active', 'woocommerce-integration' ) . '</span>';
				return $license_status_text;
			} elseif ( get_option( 'edd_' . $this->pluginSlug . '_license_status' ) == 'site_inactive' ) {
				$license_status_text = '<span style="color:red;">' . __( 'Not Active', 'woocommerce-integration' ) . '</span>';
				return $license_status_text;
			} elseif ( get_option( 'edd_' . $this->pluginSlug . '_license_status' ) == 'expired' && ( !empty( $display ) || $display != "" ) ) {
				$license_status_text = '<span style="color:red;">' . __( 'Expired', 'woocommerce-integration' ) . '</span>';
				return $license_status_text;
				...
				$license_status_text = '<span style="color:red;">' . __( 'Invalid Key', 'woocommerce-integration' ) . '</span>';
				return $license_status_text;
			} else {
				$license_status_text = '<span style="color:red;">' . __( 'Not Active', 'woocommerce-integration' ) . '</span>';
				return $license_status_text;
