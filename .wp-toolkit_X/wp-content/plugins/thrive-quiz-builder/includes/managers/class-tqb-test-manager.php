...
class TQB_Test_Manager {
	...
	public function save_test( $model ) {
		...
		if ( $test_id ) {
			...
			foreach ( $model['item_ids'] as $item_id ) {
				...
				if ( $variation && 'publish' === $variation['post_status'] ) {
	...
	public function get_test_items( $filters ) {
		...
		return $tqbdb->get_test_items( $filters );
	 ...
	 * @return array|false
	 */
	public function get_test( $model ) {
		global $tqbdb;
		...
		$model['completed_tests'] = $page->get_tests_for_page(
			array(
		...
		if ( (int) $model['status'] === 0 ) {
			$model['running_test'] = $page->get_tests_for_page(
				array(
					'page_id' => $model['page_id'],
					'status'  => 1,
				), true );
			...
			foreach ( $test_items as $key => $item ) {
				...
				if ( $key > 0 ) {
					...
					if ( is_numeric( $test_items[0]['conversion_rate'] ) ) {
						$control_rate = floatval( $test_items[0]['conversion_rate'] );
	...
	function get_test_items_data_names( $test_items = null ) {
		...
		// Get all variation IDs to fetch in one query
		$variation_ids = array();
		...
		global $tqbdb;
		$variations = $tqbdb->get_page_variations( array( 'id' => $variation_ids ), ARRAY_A );
		
		// Create lookup array for faster access
		$variation_lookup = array();
		foreach ( $variations as $variation ) {
			$variation_lookup[ $variation['id'] ] = $variation['post_title'];
		}
		
		foreach ( $test_items as $item ) {
			$result_ids[ $item['variation_id'] ]   = $item['is_control'] ? - 1 : $item['id'];
			$result_names[ $item['variation_id'] ] = isset( $variation_lookup[ $item['variation_id'] ] ) ? 
				$variation_lookup[ $item['variation_id'] ] : '';
		}
	...
	function get_test_chart_data( $interval ) {
		...
		// Sort chart data by test item order (control first, then variations)
		$sorted_chart_data = array();
		asort( $test_item_ids );
		foreach ( $test_item_ids as $main_id => $order ) {
			if ( isset( $chart_data['chart_data'][ $main_id ] ) ) {
				$sorted_chart_data[] = $chart_data['chart_data'][ $main_id ];
			}
		}
		$chart_data['chart_data'] = $sorted_chart_data;
	 ...
	 */
	public function test_item_beat_original(
		...
		if ( ! is_numeric( $variation_conversion_rate ) || ! is_numeric( ( $control_conversion_rate ) ) ) {
			return 'N/A';
		}
