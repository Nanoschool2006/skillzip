...
if ( ! class_exists( 'Thrive_Quiz_Builder' ) ) :
	...
	final class Thrive_Quiz_Builder {
		 ...
		 */
			...
			add_action( 'wp_head', array( $this, 'add_social_share_meta_tags' ), 5 );
		...
		private function includes() {
			...
			require_once( 'includes/managers/class-tqb-stats-cleanup-manager.php' );
			require_once( 'includes/managers/class-tqb-results-export-manager.php' );
			TQB_Results_Export_Manager::init();
	 ...
	 * Unschedule the auto-cleanup cron job when plugin is deactivated
	 */
	public function deactivation_hook() {
		$timestamp = wp_next_scheduled( 'tqb_auto_clear_old_stats' );
		if ( $timestamp ) {
			wp_unschedule_event( $timestamp, 'tqb_auto_clear_old_stats' );
		}
	}
	...
	/**
	 * Add Open Graph meta tags for quiz social sharing
	 * Outputs meta tags in wp_head for Facebook, Twitter, and other social platforms
	 */
	public function add_social_share_meta_tags() {
		global $post;
		...
		// Check if the post contains a quiz shortcode
		if ( ! has_shortcode( $post->post_content, self::SHORTCODE_NAME ) ) {
			return;
		}

		// Try to find quiz ID from shortcode
		preg_match( '/\[' . self::SHORTCODE_NAME . '[^\]]*id=["\']?(\d+)["\']?[^\]]*\]/', $post->post_content, $matches );
		if ( empty( $matches[1] ) ) {
			return;
		}
		...
		if ( ! empty( $image_post[0] ) ) {
			...
			$image_url = $tie_image->get_image_url();
		}
		...
		<?php endif; ?>
		<meta name="twitter:image" content="<?php echo esc_attr( $image_url ); ?>" />
	...
	public static function enqueue_frontend_scripts( $quiz_id = 0 ) {
		...
		public static function get_settings( $key = null ) {
			$settings = tqb_get_option( self::PLUGIN_SETTINGS, tqb_get_default_values( self::PLUGIN_SETTINGS ) );

			// Defensive check: ensure settings is always an array (fix for PHP 8.3 compatibility)
			if ( ! is_array( $settings ) ) {
				$settings = tqb_get_default_values( self::PLUGIN_SETTINGS );
				update_option( self::PLUGIN_SETTINGS, $settings );
			}

			// Early exit: Return all settings if no specific key requested
			if ( $key === null ) {
				return $settings;
			}

			// Return the specific key value if it exists, otherwise return null
