...
class FrmPdfsShortcodeController {
	...
	public static function frm_pdf_handler( $atts ) {
		...
		$default_atts = apply_filters(
			...
			array(
				...
				'html_id'        => '',
				'target'         => '',
				// If this includes 'admin_only', user can view all fields regardless field Visibility setting.
				'include_extras' => '',
		...
		if ( FrmPdfsAppController::FILE_MODE === $atts['mode'] ) {
			// Do not support file mode in shortcode.
			$atts['mode'] = FrmPdfsAppController::VIEW_MODE;
		}
	...
	private static function build_download_url( $shortcode_atts ) {
		...
		if ( ! empty( $shortcode_atts['public'] ) ) {
			$url_params['public']      = 1;
			$key                       = self::get_unique_pdf_key( $shortcode_atts );
			$url_params['access_code'] = FrmPdfsAccessCodeHelper::get_access_code( true, $key );
		}
	...
	private static function guest_user_can_view( $params ) {
		...
		return FrmPdfsAccessCodeHelper::verify( $params['access_code'], self::get_unique_pdf_key( $params ) );
	}

	/**
	 * Get a unique key based on the entry and view ID for a PDF.
	 * This is included in the access code to make sure each access code is unique.
	 *
	 * @since 2.0.7
	 *
	 ...
	 * @return string
	 */
	private static function get_unique_pdf_key( $params ) {
		$params_to_use = array();
		if ( ! empty( $params['id'] ) ) {
			$params_to_use['id'] = $params['id'];
		}
		if ( ! empty( $params['view'] ) ) {
			$params_to_use['view'] = $params['view'];
		}
		return md5( serialize( $params_to_use ) );
	}
