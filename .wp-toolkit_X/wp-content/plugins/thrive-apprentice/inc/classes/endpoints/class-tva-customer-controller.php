...
class TVA_Customer_Controller extends TVA_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			static::$namespace . static::$version,
			'/' . $this->base . '/hide_members_no_courses',
			array(
				array(
					'methods'             => WP_REST_Server::EDITABLE,
					'callback'            => array( $this, 'save_hide_members_no_courses_setting' ),
					'permission_callback' => array( $this, 'admin_permission_check' ),
					'args'                => array(
						'value' => array(
							'type'        => 'integer',
							'description' => 'Value for hide members no courses setting (0 or 1)',
							'required'    => true,
							'enum'        => array( 0, 1 ),
						),
					),
	...
	public function create_customer( $request ) {
		...
		// Check if user exists and if they're already a member
		if ( ! ( $existing_user instanceof WP_User ) ) {
			// User doesn't exist, safe to proceed
			$existing_user = false;
		} else {
			// User exists, check if they're already a member
			$member_exists = $this->check_if_member_exists( $existing_user );
			
			if ( $member_exists ) {
				return new WP_REST_Response( 
					array( 'message' => __( "The member already exists! If you want to update the user's access please use the Edit Access Rights option", 'thrive-apprentice' ) ), 
					405 
				);
			}
		}
	...
	public function import_customers(
		...
		foreach ( $customers as $key => $customer ) {
			$user_data = TVA_Customer_Manager::insert_customer( 
                array(
	...
	public function bulk_edit( $request ) {
		...
		switch ( $action ) {
			...
			case 'reset':
				...
				$student::remove_user_completed_meta( $student_id, $course_id );
				break;
			default:
				break;
		}
	...
	protected function get_students( $request ) {
		...
		$order_by      = ! empty( $ordering ) ? [ array_key_first( $ordering ) => array_values( $ordering )[0] ] : [ 'user_registered' => 'desc' ];
		$hide_no_courses = get_option( 'tva_hide_members_no_courses', 0 );
		...
		$args = array(
			...
			'hide_no_courses' => $hide_no_courses,
		);
	...
	/**
	 * Saves the hide members no courses setting
	 *
	 * @param WP_REST_Request $request
	 *
	 * @return WP_REST_Response
	 */
	public function save_hide_members_no_courses_setting( $request ) {
		$value = (int) $request->get_param( 'value' );
		
		// Save the setting - update_option() handles the rest
		update_option( 'tva_hide_members_no_courses', $value );
		
		return new WP_REST_Response( array( 'success' => true ), 200 );
	}

	/**
	 * Check if a WordPress user is already a member (has active orders/access)
	 * Extracted to reduce nesting in create_customer()
	 *
	 * @param WP_User $existing_user
	 *
	 * @return bool True if member exists, false otherwise
	 */
	private function check_if_member_exists( $existing_user ) {
		...
		// No orders means not a member yet
		if ( empty( $orders ) ) {
			return false;
		}
		
		// Has orders - assume member exists
		$member_exists = true;
		
		// Check for empty orders (bug fix: verify in access table if empty orders exist)
		$has_empty_order = false;
		foreach ( $orders as $order ) {
			/** @var TVA_Order $order */
		...
		// If empty orders found, verify actual access in history table
		if ( $has_empty_order ) {
