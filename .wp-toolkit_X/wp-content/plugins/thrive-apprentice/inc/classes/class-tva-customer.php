...
class TVA_Customer implements JsonSerializable {
	...
	const HAS_COMPLETED_COURSE = 'tva_completed_course_';
	...
	/**
	 * Returns an array of learned lessons.
	 *
	 * @return array
	 */
	public function get_learned_items() {
		if ( empty( $this->learned_items ) ) {
			$this->_data['learned_items'] = tva_get_learned_lessons();
		}
	...
	/**
	 * Returns the count of assessments completed by the user.
	 *
	 * @param TVA_Course_V2|int $course
	 *
	 * @return int
	 */
	public function get_user_assessments_completed( $course ) {
		if ( is_int( $course ) ) {
			$course = new TVA_Course_V2( $course );
		}

		$assessments_done = 0;
		if ( ! empty( $assessment_items = $course->get_published_assessments() ) ) {

			/**
			 * There can be multiple assessments per course.
			 * We need to return only one for each to be counted as completed
			 * to prevent a single assessment from filling the count for others.
			 */
			foreach ( $assessment_items as $assessment ) {

				$assessment_id = $assessment->__get( 'ID' );
				switch ( Grading_Base::get_assessment_completion_credit( $assessment_id ) ) {
					case 'upon_passing':
						$assessment_item = get_user_meta(
							$this->__get( 'ID' ), // User ID.
							TVA_User_Assessment::ASSESSMENT_PASSED . $assessment_id,
							true
						) ? 1 : 0;
					...
					case 'upon_grading':
						$assessment_item = get_user_meta(
							$this->__get( 'ID' ), // User ID.
							TVA_User_Assessment::ASSESSMENT_GRADED . $assessment_id,
							true
						) ? 1 : 0;

						break;
					default:
						// upon_submission or empty.
						$assessment_item = get_user_meta(
							$this->__get( 'ID' ), // User ID.
							TVA_User_Assessment::SUBMISSION_COUNTER_META . $assessment_id,
							true
						) ? 1 : 0;
				...
				$assessments_done += $assessment_item;
			}
		}

		return $assessments_done;
	}
	...
	/**
	 * Returns an array of course lessons.
	 * 
	 * @param int $course_id
	 *
	 * @return array
	...
	public function get_course_progress_status( $course ) {
		...
		// If user has previously completed the course, skip calculations.
		if ( self::get_user_completed_meta( $this->get_id(), $course->get_id() ) ) {
			return TVA_Const::TVA_COURSE_PROGRESS_COMPLETED;
		}
	...
	/**
	 * Get tva_completed_course_ + course_id user meta if user has completed the course.
	 *
	 * @return mixed
	 */
	public static function get_user_completed_meta( $user_id, $course_id ) {
		return get_user_meta( $user_id, self::HAS_COMPLETED_COURSE . $course_id, true );
	}

	/**
	 * Add tva_completed_course_ + course_id user meta if user has completed the course.
	 *
	 * @return void
	 */
	public static function add_user_completed_meta( $user_id, $course_id ) {
		add_user_meta( $user_id, self::HAS_COMPLETED_COURSE . $course_id, true, true );
	}

	/**
	 * Remove the tva_completed_course_ + course_id user meta
	 *
	 * @return void
	 */
	...
	public function trigger_course_purchase( $order, $initiator = '' ) {
		...
		foreach ( $order->get_order_items() as $order_item ) {
			...
			} else if ( $order->is_square() ) {
				$products = TVA_Square_Integration::get_all_products_for_identifier( $order_item->get_product_id() );
			} else {
	...
	public static function get_students( $filters = [] ) {
		$hide_no_courses = ! empty( $filters['hide_no_courses'] ) ? (int) $filters['hide_no_courses'] : 0;

		// When hiding members with no courses, filter by published_courses_count > 0
		if ( $hide_no_courses ) {
			return TVA_Customer::get_students_with_published_courses( $filters );
		}
		
		// When NOT hiding, use original logic (which already filters by SUM(status) > 0)
		// This shows all users with active course access (regardless of published status)
		$students = History_Table::get_instance()->get_all_students( $filters );
		
		// Get total by fetching all students without pagination
		$all_filters = $filters;
		unset( $all_filters['limit'] );
		$all_students = History_Table::get_instance()->get_all_students( $all_filters );
		$total = count( $all_students );
		
		$items = [];

		foreach ( $students as $student_data ) {
			$items[] = new static( $student_data );
		}
	...
	/**
	 * Get students who have published courses (for hide_no_courses functionality)
	 * 
	 * Optimized approach:
	 * 1. Get students with active courses from history table (single query)
	 * 2. Filter by published courses count
	 ...
	 * @return array ['items' => TVA_Customer[], 'total' => number]
	 */
	public static function get_students_with_published_courses( $filters = [] ) {
		$limit = $filters['limit'] ?? ['offset' => 0, 'limit' => 10];
		$offset = (int) $limit['offset'];
		$page_limit = (int) $limit['limit'];
		
		// Get ALL students from history table (without pagination)
		// We need to do this because we filter by published_courses_count which requires
		// checking each student individually
		$all_filters = $filters;
		unset( $all_filters['limit'] );
		
		$all_students = History_Table::get_instance()->get_all_students( $all_filters );
		
		// Filter to only students with published courses
		$filtered_students = [];
		
		foreach ( $all_students as $student_data ) {
			$student = new static( $student_data );
			$published_count = $student->get_published_courses_count();
			
			// Only include students who have at least one published course they can access
		...
		$total = count( $filtered_students );
		$paginated_items = array_slice( $filtered_students, $offset, $page_limit );
	...
	public function bulk_reset_items( $items_ids, $course_id ) {
		...
		$this->bulk_reset_drip_content_unlocked( $items_ids );
	...
	/**
	 * Removes the items received as param from the 'tva_drip_content_unlocked' user meta
	 * so they will respect the drip conditions set again
	 *
	 * @param array $items_ids Items can be lessons and modules as well
	 *
	 * @return void
	 */
	public function bulk_reset_drip_content_unlocked( $items_ids ) {
		$drip_content_unlocked = $this->get_drip_content_unlocked();
		
		// Remove the items from the unlocked array
		$drip_content_unlocked = array_diff( $drip_content_unlocked, $items_ids );
		
		// Re-index the array to ensure it's a proper sequential array
		$drip_content_unlocked = array_values( $drip_content_unlocked );
		
		update_user_meta( $this->get_id(), 'tva_drip_content_unlocked', $drip_content_unlocked );
		
		// Also update the internal cache
		$this->_drip_content_unlocked = $drip_content_unlocked;
	}
	...
	public function bulk_delete_assessments( $module, $course_id ) {
		...
		foreach ( $assessments as $assessment ) {
			...
			if ( isset( $submitted_assessments[ $course_id ][ $assessment ] ) ) {
		 ...
		 */
		if ( isset( $submitted_assessments[ $course_id ] ) && is_array( $submitted_assessments[ $course_id ] ) ) {
	...
	public function bulk_reset_bypassed_items( $items_ids, $course_id ) {
		...
		$course_bypassed_items = $bypassed_items && isset( $bypassed_items[ $course_id ] ) ? $bypassed_items[ $course_id ] : [];
	...
	public function bulk_reset_completed_lessons( $items_ids, $course_id ) {
		...
		$course_learned_lessons = $learned_lessons && isset( $learned_lessons[ $course_id ] ) ? $learned_lessons[ $course_id ] : [];
	...
	public function prepare_data() {
		...
		foreach ( $products as $product ) {
			...
			// Process each course in the product
			foreach ( $product->get_courses() as $course ) {
				...
				// Skip if course already in array
				if ( $this->is_course_in_array( $course->ID ) ) {
					continue;
				}

				// Add course and calculate progress/locked status
	...
	public function get_courses() {
		...
		foreach ( $products as $product ) {
			...
			// Add accessible courses that aren't already in the array
			foreach ( $product->get_courses() as $course ) {
				if ( $this->is_course_in_array( $course->ID ) ) {
					continue;
				}
	 ...
	 * This method checks access based on the access history table to ensure
	 * we only count courses with active access (status > 0), excluding expired courses.
	 * 
	 * Optimized to use a single query to get all active courses for the user.
	 */
	public function get_published_courses_count() {
		global $wpdb;
		
		// Single optimized query to get all active course IDs for this user
		// Using esc_sql() for table prefix as it comes from WordPress configuration
		$table_name = esc_sql( $wpdb->prefix ) . 'tva_access_history';
		$query = $wpdb->prepare(
			"SELECT course_id, SUM(status) as status 
			FROM {$table_name} 
		...
		$active_courses = $wpdb->get_results( $query, ARRAY_A );
		
		// If no active courses, return 0
		if ( empty( $active_courses ) ) {
			$this->_data['published_courses_count'] = 0;
			return 0;
		}
		
		// Build array of active course IDs for quick lookup
		// Filter out any NULL values and convert to integers
		$active_course_ids = array_filter( array_map( 'intval', array_column( $active_courses, 'course_id' ) ) );
		$active_course_map = array_flip( $active_course_ids );
		
		// Get all courses from products (cached)
		$products = Product::get_items_from_cache();
		
		// Flatten all courses from all products to avoid nested loops
		$all_courses = [];
		foreach ( $products as $product ) {
			$all_courses = array_merge( $all_courses, $product->get_courses_from_cache() );
		}
		
		// Count unique published courses that are in active list
		$published = 0;
		$counted_course_ids = []; // Track which course IDs we've already counted
		
		foreach ( $all_courses as $course ) {
			// Skip if already counted
			if ( isset( $counted_course_ids[ $course->ID ] ) ) {
				continue;
			}
			
			// Skip if not in active list
			if ( ! isset( $active_course_map[ $course->ID ] ) ) {
				continue;
			}
			...
			// Count this course
			$published++;
			$counted_course_ids[ $course->ID ] = true;
			
			// Also add to courses array for later use
			if ( ! $this->is_course_in_array( $course->ID ) ) {
				$this->_data['courses'][] = $course;
			}
		}
	 ...
	 * @param TVA_Course_V2 $course
	 *
	 * @return array ['progress' => string, 'modules' => array]
	 */
	public function calculate_progress( $course ) {
		// Get user's completed lessons and assessments (extract once to avoid duplication)
		$lessons_completed = $this->get_meta( 'tva_learned_lessons' );
		$lessons_completed = is_array( $lessons_completed ) && array_key_exists( $course->get_id(), $lessons_completed ) 
			? $lessons_completed[ $course->get_id() ] 
			: [];
		
		$assessments_completed = $this->get_user_assessments_completed( $course );
		$completed = count( $lessons_completed ) + $assessments_completed;
		$total = $course->get_published_lessons_count() + $course->get_published_assessments_count();
		
		if ( empty( $lessons_completed ) || $total === 0 ) {
			return array(
				'progress' => '0%',
		...
		$progress_percentage = Dynamic_Actions\tcb_tva_dynamic_actions()->get_progress_by_type( 'course', $completed, $total );
		$is_currently_complete = ( 100 === (int) $progress_percentage );
		$was_completed = self::get_user_completed_meta( $this->get_id(), $course->get_id() );
		
		if ( $was_completed && ! $is_currently_complete ) {
			self::remove_user_completed_meta( $this->get_id(), $course->get_id() );
			return array(
		...
		if ( $is_currently_complete && ! $was_completed ) {
			self::add_user_completed_meta( $this->get_id(), $course->get_id() );
			return array(
				'progress' => '100%',
				'modules'  => $this->calculate_modules_progress( $course ),
			);
	...
	public function get_course_certificates() {
		...
		foreach ( $courses as $course ) {
			$is_course_complete = tva_get_course_progress( $course, $this->get_id() ) === '100%';
			if ( $course->has_certificate() && $is_course_complete ) {
