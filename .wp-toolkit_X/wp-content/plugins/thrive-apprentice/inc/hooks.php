...
/**
 * Migrate existing courses to have welcome email disabled by default.
 *
 * This migration runs once to ensure all existing courses (created before this feature)
 * have the welcome email setting explicitly set to disabled.
 */
add_action( 'admin_init', static function () {
	$migration_option = 'tva_course_welcome_email_migration';
	...
	foreach ( $courses as $course ) {
		$existing_value = get_term_meta( $course->get_id(), TVA_Course_V2::SEND_WELCOME_EMAIL_META_KEY, true );

		// Only set to false if the meta key doesn't exist (empty string means it doesn't exist)
		if ( $existing_value !== '' ) {
			continue;
		}

		update_term_meta( $course->get_id(), TVA_Course_V2::SEND_WELCOME_EMAIL_META_KEY, false );
	}

	update_option( $migration_option, '1', 'no' );
} );
...
add_action( 'pre_get_posts', static function ( $query ) {
	...
	if ( $query->is_tax( TVA_Const::COURSE_TAXONOMY ) && isset( $query->query['tva_courses'] ) ) {
...
add_action( 'save_post', static function ( $post_ID, $post, $update ) {
	if ( in_array( $post->post_type, [ TVA_Const::LESSON_POST_TYPE, TVA_Const::ASSESSMENT_POST_TYPE, TVA_Const::CHAPTER_POST_TYPE, TVA_Const::MODULE_POST_TYPE ] ) ) {
		...
		if ( $course instanceof TVA_Course_V2 ) {
			...
			// Clear stats cache when lessons/assessments are saved
...
add_action( 'before_delete_post', static function ( $post_id, $post ) {
	if ( in_array( $post->post_type, [ TVA_Const::LESSON_POST_TYPE, TVA_Const::ASSESSMENT_POST_TYPE, TVA_Const::CHAPTER_POST_TYPE, TVA_Const::MODULE_POST_TYPE ] ) ) {
		...
		if ( $course instanceof TVA_Course_V2 ) {
			...
			delete_term_meta( $course->get_id(), 'tva_assessments_count' );
			delete_term_meta( $course->get_id(), 'tva_items_count' );
			// Clear stats cache when lessons/assessments are deleted
			TVA_Course_V2::clear_courses_cache();
		}
	}
}, 10, 2 );
...
add_filter( 'tva_can_be_marked_as_completed', static function ( $allowed = true, $config = array(), $post_id = 0 ) {
	if ( $allowed && is_array( $config ) && ( isset( $config['video_progress'] ) && true === $config['video_progress'] ) ) {
...
add_action( 'admin_init', function() {
	if ( class_exists( 'TVA\Stripe\Webhook_Update' ) && Webhook_Update::should_update() ) {
		$live_updated = Webhook_Update::update();

		// Update the test mode webhook if the live mode webhook was updated successfully.
		if ( $live_updated ) {
			Webhook_Update::update( false );
		}
	}
} );
