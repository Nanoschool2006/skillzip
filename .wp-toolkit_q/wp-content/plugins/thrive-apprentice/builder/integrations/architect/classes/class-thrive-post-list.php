...
class Thrive_Post_List extends TCB_Post_List {
	...
	public function __construct( $attr = [], $article = '' ) {
		...
		if ( ! empty( $attr['reset_query'] ) || is_search() ) {
			unset( $attr['reset_query'], $attr['query'] );
		}
	...
	public static function thrive_theme_template_copied_data( $destination_template, $source_template ) {
		...
		if ( isset( $destination_sections['content'] ) && isset( $source_sections['content'] ) ) {
			/* Check if source content is an array with a 'content' key */
			if ( is_array( $source_sections['content'] ) && isset( $source_sections['content']['content'] ) ) {
				$source_content = $source_sections['content']['content'];
			} elseif ( is_string( $source_sections['content'] ) ) {
				/* If source content is a string, use it directly */
				$source_content = $source_sections['content'];
			} else {
				/* Fallback to empty string if content structure is unexpected */
				$source_content = '';
			}
			
			/* Only proceed if we have valid source content */
			if ( ! empty( $source_content ) ) {
				$updated_content = str_replace( '[thrive_blog_list', '[thrive_blog_list reset_query=1', $source_content );
				
				/* Update the destination content */
				if ( is_array( $destination_sections['content'] ) ) {
					$destination_sections['content']['content'] = $updated_content;
				} else {
					$destination_sections['content'] = $updated_content;
				}
			}
		}
