...
class Video_Data extends Event {
	...
	public static function label(): string {
		return esc_html__( 'Video watch data', 'thrive-apprentice' );
	}
	...
	public static function register_action() {
		add_action( 'thrive_video_update_watch_data', static function ( $data ) {
			...
			if ( isset( $data['range_start'] ) && isset( $data['range_end'] ) ) {
				...
				// Fixes skipping issue when user suddenly skips from start to the end of the video.
				if ( $start === $end ) {
					return;
				}
	...
	public function get_event_description(): string {
		...
		return sprintf( esc_html__( ' continued watching video "%s" from lesson "%s", course "%s"', 'thrive-apprentice' ), $video, $lesson, $course );
	}
}
