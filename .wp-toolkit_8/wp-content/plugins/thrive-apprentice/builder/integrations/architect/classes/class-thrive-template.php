...
class Thrive_Template {
	...
	public function copy_data_from( $template_id ) {
		if ( ! empty( $template_id ) && $this->ID !== (int) $template_id ) {
			...
			/* Duplicate conditional display groups after all other template copying operations */
			$this->duplicate_conditional_display_groups( $template_id );
		}
	}
	...
	/**
	 * Duplicate conditional display groups from source template to the new template
	 * Since conditional displays are stored separately and loaded via AJAX, we need to handle them differently
	 *
	 * @param int $source_template_id
	 */
	private function duplicate_conditional_display_groups( $source_template_id ) {
		/* Only proceed if the Conditional_Display_Group class exists (TCB is active) */
		if ( ! class_exists( '\TCB\ConditionalDisplay\PostTypes\Conditional_Display_Group' ) ) {
			return;
		}

		/* Get all template content that might contain conditional display references */
		$content_fields = [ 'structure', 'sections', 'tve_updated_post', 'style', 'tve_custom_css', 'tve_user_custom_css' ];
		$all_display_group_keys = [];
		
		/* First pass: collect all display group keys and clone the groups */
		foreach ( $content_fields as $field ) {
			$content = $this->meta( $field );
			
			if ( ! empty( $content ) ) {
				/* Find conditional display group references and clone them */
				$content = $this->clone_conditional_groups_in_template_content( $content, $all_display_group_keys );
				
				/* Update the template with the new content */
				$this->set_meta( $field, $content );
			}
		}

		/* Second pass: replace all remaining instances of old keys with new keys */
		if ( ! empty( $all_display_group_keys ) ) {
			
			foreach ( $content_fields as $field ) {
				$content = $this->meta( $field );
				
				if ( ! empty( $content ) ) {
					/* Recursively process both string and array content */
					$content = $this->replace_keys_in_array_content( $content, $all_display_group_keys );
	...
	/**
	 * Clone conditional display groups found in template content and update references
	 *
	 * @param mixed $content The content to scan and update (can be string or array)
	 * @param array &$display_group_keys Reference to collect all display group key mappings
	 * @return mixed Updated content with new group IDs
	 */
	private function clone_conditional_groups_in_template_content( $content, &$display_group_keys = [] ) {
		if ( ! class_exists( '\TCB\ConditionalDisplay\PostTypes\Conditional_Display_Group' ) ) {
			return $content;
		}

		if ( is_string( $content ) ) {
			/* Use the existing method from TCB to clone conditional groups in content */
			$cloned_result = \TCB\ConditionalDisplay\PostTypes\Conditional_Display_Group::clone_conditional_groups_in_content( $content );
			$content = $cloned_result['content'];
			
			/* Collect display group keys for later replacement */
			if ( ! empty( $cloned_result['display_group_keys'] ) ) {
				$display_group_keys = array_merge( $display_group_keys, $cloned_result['display_group_keys'] );
			}
		} elseif ( is_array( $content ) ) {
			/* Recursively process array content (like sections) */
			foreach ( $content as $key => $value ) {
				$content[ $key ] = $this->clone_conditional_groups_in_template_content( $value, $display_group_keys );
			}
		}
	...
	/**
	 * Recursively replace display group keys in array content
	 *
	 * @param mixed $content The content to process (can be array, string, or other types)
	 * @param array $display_group_keys Array of old_key => new_key mappings
	 * @return mixed Updated content
	 */
	private function replace_keys_in_array_content( $content, $display_group_keys ) {
		if ( is_string( $content ) ) {
			$original_content = $content;
			foreach ( $display_group_keys as $old_key => $new_key ) {
				$content = str_replace( $old_key, $new_key, $content );
			}
		} elseif ( is_array( $content ) ) {
			$new_content = [];
			foreach ( $content as $key => $value ) {
				/* Check if the key itself needs to be replaced */
				$new_key = $key;
				foreach ( $display_group_keys as $old_display_key => $new_display_key ) {
					if ( $key === $old_display_key ) {
						$new_key = $new_display_key;
						break;
					}
				}
				
				/* Special handling for conditional display groups array */
				if ( $key === 'conditional-display-groups' && is_array( $value ) ) {
					$new_value = [];
					foreach ( $value as $group_index => $group_data ) {
						if ( is_array( $group_data ) && isset( $group_data['key'] ) ) {
							/* Check if this group's key needs to be updated */
							$old_group_key = $group_data['key'];
							if ( isset( $display_group_keys[ $old_group_key ] ) ) {
								$group_data['key'] = $display_group_keys[ $old_group_key ];
							}
							/* Also update any references within the group data */
							$group_data = $this->replace_keys_in_array_content( $group_data, $display_group_keys );
						}
						$new_value[ $group_index ] = $group_data;
					}
					$value = $new_value;
				} else {
					/* Process the value recursively */
					$value = $this->replace_keys_in_array_content( $value, $display_group_keys );
				}
