...
class TVA_Order {
	...
	public function is_square() {
		return strtolower( $this->get_gateway() ) === strtolower( TVA_Const::SQUARE_GETWAY );
	}
	...
	public static function get_orders_by_payment_id ( $payment_id, $status ) {
		global $wpdb;

		$orders_table = $wpdb->prefix . TVA_Const::DB_PREFIX . TVA_Const::ORDERS_TABLE_NAME;
		$sql          = 'SELECT * FROM ' . $orders_table . ' WHERE payment_id = %s AND status = %s GROUP BY user_id';

		return $wpdb->get_results( $wpdb->prepare( $sql, $payment_id, $status ), ARRAY_A );
	}
}
