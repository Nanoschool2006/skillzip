...
class TVA_Settings_Manager implements JsonSerializable {
	...
	public function add_login_element( $page_id = 0 ) {
		...
		// Ensure we have valid post meta values
		$save_post    = is_string( $save_post ) ? $save_post : '';
		$updated_post = is_string( $updated_post ) ? $updated_post : '';
		...
		if ( ! is_wp_error( $cloud_data ) && strpos( $updated_post, 'thrv-login-element' ) === false ) {
			...
			// Get existing CSS and append new CSS instead of replacing it
			$existing_css = get_post_meta( $page_id, 'tve_custom_css', true );
			...
			update_post_meta( $page_id, 'tve_custom_css', $combined_css );
		}
	}
