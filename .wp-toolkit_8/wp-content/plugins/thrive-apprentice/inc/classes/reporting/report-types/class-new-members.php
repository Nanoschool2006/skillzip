...
class New_Members extends Report_Type {
	...
	public static function count_data( $query = [] ): int {
		...
		return (int) History_Table::get_instance()->get_total_students( $query['filters'], true );
	}
	...
	public static function get_chart_data( $query = [] ): array {
		...
		foreach ( History_Table::get_instance()->get_students( $query['filters'], true ) as $item ) {

			$items[] = [
				'date'  => $item['user_registered'],
				'count' => (int) $item['number'],
			];

			if ( empty( $labels['date']['values'][ $item['user_registered'] ] ) ) {
				$labels['date']['values'][ $item['user_registered'] ] = Created::format_value( $item['user_registered'] );
			}
		}
	...
	public static function get_table_data( $query ): array {
		...
		foreach ( $items as &$item ) {
			...
			$item['date'] = $item['user_registered'];

			unset( $item['active'], $item['inactive'], $item['user_registered'] );
		}

		$number_of_items = count( $items );

		if ( empty( $query['order_by'] ) ) {
			$query['order_by'] = 'user_registered';
		}
