...
class Product implements JsonSerializable {
	...
	public function jsonSerialize() {
		return array_merge(
			...
			[
				...
				'stripe_manageable'   => (bool) $this->is_stripe_manageable(),
				'term'                => $this->_term,
				'rules'               => $this->rules,
	 ...
	 * @return boolean
	 */
	public function is_stripe_manageable() {
		if ( empty( $this->_data['stripe_manageable'] ) ) {
			$this->_data['stripe_manageable'] = $this->get_meta( 'tva_stripe_manageable' );
		}

		return $this->_data['stripe_manageable'];
	}
	...
	public function get_access_expiry() {
		...
		if ( ! isset( $this->_data['access_expiry']['redirect'] ) ) {
			$this->_data['access_expiry']['redirect'] = array();
		}

		// Ensure value key exists
		if ( ! isset( $this->_data['access_expiry']['redirect']['value'] ) ) {
			$this->_data['access_expiry']['redirect']['value'] = '';
		}
		...
		if ( ! empty( $this->_data['access_expiry'] ) && ! empty( $this->_data['access_expiry']['redirect'] ) && is_numeric( $this->_data['access_expiry']['redirect']['value'] ) ) {
			...
			$this->_data['access_expiry']['redirect']['preview_url'] = get_permalink( $this->_data['access_expiry']['redirect']['value'] );
			$this->_data['access_expiry']['redirect']['edit_url']    = add_query_arg(
				array(
					'post'   => $this->_data['access_expiry']['redirect']['value'],
					'action' => 'architect',
					'tve'    => true,
				),
				admin_url( 'post.php' )
			...
			$this->_data['access_expiry']['redirect']['title']       = $this->_data['access_expiry']['redirect']['value'];
			$this->_data['access_expiry']['redirect']['preview_url'] = $this->_data['access_expiry']['redirect']['value'];
			$this->_data['access_expiry']['redirect']['edit_url']    = '';
		}
	...
	public function save() {
		...
		if ( isset( $this->_data[ 'stripe_manageable' ] ) ) {
			update_term_meta( $this->_term->term_id, 'tva_stripe_manageable', $this->_data[ 'stripe_manageable' ] );
		}

		// Force WP-Cron to run immediately after product update.
		spawn_cron();
	...
	public function get_buy_link( $provider = '' ) {
		...
		if ( $provider === 'square' ) {
			$mode = get_term_meta( $this->get_id(), 'tva_square_mode', true ) ?? 0;
