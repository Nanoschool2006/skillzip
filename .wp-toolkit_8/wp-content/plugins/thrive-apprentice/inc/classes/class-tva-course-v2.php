 ...
 * @property TVA_Course_Grade      		  $grade
 ...
 * @property array|TVA_Assessment         $published_assessments
...
class TVA_Course_V2 extends TVA_Course implements JsonSerializable {
	...
	const SEND_WELCOME_EMAIL_META_KEY = 'tva_send_course_welcome_email';
	...
	protected function _insert() {
		...
		// New courses have welcome email enabled by default
		update_term_meta( $this->term_id, self::SEND_WELCOME_EMAIL_META_KEY, true );

		// Clear course cache after insert
		static::clear_courses_cache();
	...
	public function _update() {
		...
		if ( isset( $this->_data['send_welcome_email'] ) ) {
			update_term_meta( $this->term_id, self::SEND_WELCOME_EMAIL_META_KEY, (bool) $this->_data['send_welcome_email'] );
		}
	...
	public function update_slug( $slug ) {
		...
		$result = wp_update_term( $term->term_id, TVA_Const::COURSE_TAXONOMY, [ 'slug' => $slug ] );

		// Clear course cache after slug update
		if ( ! is_wp_error( $result ) ) {
			static::clear_courses_cache();
		}
	...
	public function delete() {
		...
		if ( $result ) {
			...
			// Clear course cache after delete
			static::clear_courses_cache();
		}
	...
	public function duplicate() {
		...
		if ( $this->has_grade() instanceof WP_Post ) {
			$this->get_grade( true )->duplicate( $new_course );
		}
	...
	/**
	 * Clear all course transients
	 * Called whenever a course is created, updated, or deleted
	 *
	 * @return void
	 */
	public static function clear_courses_cache() {
	...
	/**
	 * Get the current cache version used to namespace course transient keys
	 *
	 * @return int
	 */
	public static function get_courses_cache_version() {
		return (int) get_option( 'tva_courses_cache_version', 0 );
	}
	...
	public static function get_items( $args = array(), $count = false ) {
		...
		// Generate a unique cache key based on arguments and cache version
		$cache_key = 'tva_courses_v' . static::get_courses_cache_version() . '_' . md5( serialize( $args ) . ( $count ? '_count' : '' ) );
		...
		if ( ! empty( $args['labels'] ) || ! empty( $args['free_for_all'] ) ) {
			...
			$arguments['meta_query']['tva_label'] = $labels;
		}
		...
		if ( true === $count ) {
			...
			$cache_expiration = apply_filters( 'tva_courses_cache_expiration', HOUR_IN_SECONDS );
			set_transient( $cache_key, $result, $cache_expiration );
			return $result;
		}
		...
		// Cache the results with filterable expiration time
		$cache_expiration = apply_filters( 'tva_courses_cache_expiration', HOUR_IN_SECONDS );
	...
	public function get_label_data() {
		...
		$default_labels = get_option( 'tva_filter_labels', array() );
		$default_labels = isset( $default_labels[1] ) ? $default_labels : TVA_Const::default_labels();
		$label_id = $this->get_label_id();

		if ( empty( $label ) || ! is_array( $label ) ) {
			if ( is_user_logged_in() && isset( $default_labels[$label_id] )) {
				$label = array(
					'title'         => $default_labels[ $label_id ]['title'],
					'color'         => $default_labels[ $label_id ]['color'],
					'ID' 			=> $label_id,
				);
			} else {
				$label = array(
	...
	public static function delete_count_enrolled_users_cache( $course_id = 0 ) {
		...
		// Also clear the transient cache for course statistics
		static::clear_courses_cache();
	}
	...
	/**
	 * Check if welcome email should be sent for this course.
	 *
	 * Returns false for existing courses that were created before this feature
	 * (meta key doesn't exist), and the stored value for newer courses.
	 *
	 * @return bool
	 */
	public function get_send_welcome_email() {
		$value = get_term_meta( $this->get_id(), self::SEND_WELCOME_EMAIL_META_KEY, true );

		// If meta doesn't exist, default to disabled (for existing courses)
		if ( $value === '' ) {
			return false;
		}
	...
	/**
	 * Set whether welcome email should be sent for this course.
	 *
	 * @param bool $enabled Whether to enable welcome emails for this course.
	 *
	 * @return bool|int Meta ID if the key didn't exist, true on update, false on failure.
	 */
	public function set_send_welcome_email( $enabled ) {
		return update_term_meta( $this->get_id(), self::SEND_WELCOME_EMAIL_META_KEY, (bool) $enabled );
	}
	...
	public function get_published_assessments() {
		$published_assessments = [];

		if ( ! isset( $this->_data['published_assessments'] ) ) {
			foreach( $this->get_published_items() as $item ) {
				if ( $item->post_type === TVA_Const::ASSESSMENT_POST_TYPE ) {
					$published_assessments[] = $item;
				}
			}

			$this->_data['published_assessments'] = $published_assessments;
		}

		return $this->_data['published_assessments'];
	}
	...
	public function has_grade () {
		$post = get_post( get_term_meta( $this->term_id, 'tva_grade', true ) );
		return $post instanceof WP_Post ? $post : false;
	}
	...
	public function get_grade( $force_ensure = false ) {
		$grade = tva_course_grade()->set_course( $this );
		$grade->ensure_post($force_ensure);
		return $grade;
	}
	...
	public function to_array() {
		return array_merge(
			...
			'send_welcome_email'      => $this->get_send_welcome_email(),
			'level'                   => $this->get_level_id(),
			'label'                   => $this->get_label_id(),
			'text'                    => $this->name,
			...
			'has_grade' 	          => (int) ( $this->has_grade() instanceof WP_Post ),
	...
	public function publish() {
		...
		if ( $status === 'publish' ) {
			/**
			 * Fires after a course transitions to published status.
			 *
			 * @param TVA_Course_V2 $course The published course instance.
			 */
			try {
				do_action( 'tva_course_published', $this );
			} catch ( \Exception $e ) {
				error_log( 'TVA: Error during tva_course_published action for course ' . $this->get_id() . ': ' . $e->getMessage() );
			}
		}
