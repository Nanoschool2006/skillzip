		...
		init () {
			if ( ! tcb.Utils.isEditorPage() ) {
				...
				if ( window.ThriveAppFront && ThriveAppFront.assessment_submitted && ! parseInt( ThriveAppFront.assessment_submitted ) ) {
					...
					// Hide the button until the user has completed the assessment.
					$completionPageButton = $( '[data-shortcode-id="call_to_action"][data-dynamic-link="tva_dynamic_actions_link"]' ).closest( '.thrv-button' );
					$completionPageButton.addClass( 'tcb-permanently-hidden' );
				}
			}
		},
		/**
