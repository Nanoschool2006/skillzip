...
class Shortcodes {
	...
	public function dynamic_element( $attr = array(), $content = '' ) {
		...
		// Check if video progress_enabled is not null then check if video progress checking is enabled.
		$video = null;
		if ( tcb_tva_dynamic_actions()->get_active_object() ) {
			$video = tcb_tva_dynamic_actions()->get_active_object()->get_video();
		}

		if (
			! is_user_logged_in() || 
			( ! is_null( $video ) && ! is_null( $video->progress_enabled ) && ! $video->progress_enabled )
		) {
	...
	public function course_progress( $attr = array() ) {
		$course = tcb_tva_dynamic_actions()->get_active_course();
    
		if ( ! $course ) {
			return '0%';
		}
		
		$course_progress = tva_get_course_progress( $course );
		
		return TVD_Global_Shortcodes::maybe_link_wrap( $course_progress, $attr );
	}
