...
class Main {
	...
	public function get_next_item_text() {
		...
		if ( ( $this->active_object instanceof TVA_Lesson || $this->active_object instanceof TVA_Assessment ) && ! $this->has_next_item() ) {
			 ...
			 */
			if ( 
				$this->maybe_go_to_completed_page() || 
				( 
					$this->active_object->is_completed() && 
					$this->active_course->has_completed_post() && 
					$this->active_course->get_completed_post()->is_valid() && 
					$this->active_object instanceof TVA_Assessment 
				) 
			) {
	...
	public function get_next_lesson_link() {
		...
		if ( $next_item instanceof TVA_Lesson || $next_item instanceof TVA_Assessment ) {
			...
			if ( 
				...
				$url = $this->active_course->get_completed_post( true )->url;
			} else {
	...
	public function get_mark_as_complete_next_link() {
		if ( $this->active_course->has_access() && $this->maybe_go_to_completed_page() ) {
			$url = $this->active_course->get_completed_post()->url;
		} else {
			$url = $this->get_next_lesson_link();
		}
	...
	public function get_call_to_action_link() {
		...
		if ( $this->should_display_buy_link() ) {
			...
			if ( empty( $this->active_object ) ) {
				...
				$course_items_count = (int) $this->active_course->get_published_lessons_count() + (int) $this->active_course->get_published_assessments_count();
				$completed_items_count = (int) $customer->get_count_course_completed_lessons( $this->active_course->get_id() ) + 
										 (int) $customer->get_user_assessments_completed( $this->active_course );

				if ( $published_lessons_count > 0 && ( $completed_items_count === 0 || $course_items_count <= $completed_items_count ) ) {
