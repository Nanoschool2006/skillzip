...
class TCB_Hooks {
	...
	public static function init() {
		 ...
		 */
		add_filter( 'architect.branding', array( __CLASS__, 'architect_branding' ), 10, 2 );
	...
	public static function tqb_add_script_to_main_frame() {
		...
		if ( self::is_editable( $type ) ) {
			...
			if ( ! is_array( $quiz_type ) || empty( $quiz_type['type'] ) ) {
				$quiz_type = array( 'type' => '' );
			}
			...
			// Fetch quiz categories for personality quizzes
			$quiz_results   = array();
			$has_categories = false;
			if ( $quiz_type['type'] === Thrive_Quiz_Builder::QUIZ_TYPE_PERSONALITY ) {
				$quiz_manager = new TQB_Quiz_Manager( $variation['quiz_id'] );
				$results      = $quiz_manager->get_results();

				if ( ! empty( $results ) && is_array( $results ) ) {
					foreach ( $results as $result ) {
						if ( ! empty( $result['id'] ) && ! empty( $result['text'] ) ) {
							$quiz_results[] = array(
								'id'   => (int) $result['id'],
								'name' => $result['text'],
							);
					...
					$has_categories = ! empty( $quiz_results );
				}
			}
...
//			add_action( 'wp_footer', 'tve_output_wysiwyg_editor' ); TODO: research this!
	...
	/**
	 * Handles quiz share URLs and redirects
	 * 
	 * 1. Loads tqb_quiz.php template for social media crawlers
	 * 2. Redirects users coming from Facebook to the actual quiz page
	 */
	public static function tqb_quiz_quiz_page_redirect() {
		// Check: Is this a quiz share URL with required parameters?
		if ( ! empty( $_GET['post_type'] ) && $_GET['post_type'] === Thrive_Quiz_Builder::SHORTCODE_NAME
		     && ! empty( $_GET['tqb_quiz_id'] )
		     && ! empty( $_GET['tqb_redirect_post_id'] )
		     && ! empty( $_GET['image_url'] ) ) {

			// Load the share template for ALL requests (social crawlers and users)
			// This ensures Twitter always sees the proper meta tags
			$quiz_layout = tqb()->plugin_path( 'tcb-bridge/editor-templates/tqb_quiz.php' );
			if ( file_exists( $quiz_layout ) ) {
				include $quiz_layout;
				exit();
		...
		// Backward compatibility: Old redirect logic
		if ( ! empty( $_GET['tqb_redirect_post_id'] )
		     ...
		     && empty( $_GET['image_url'] ) // Only redirect if NOT a new share URL
			/**
	 ...
	 * @return string
	 */
	public static function architect_branding( $string, $type = 'logo_src' ) {

		if ( self::is_editable( get_post_type() ) ) {
			switch ( $type ) {
				case 'name':
					$string = 'Thrive Quiz Builder';
					break;
				default:
					$string = tqb()->plugin_url( 'assets/images/tqb-logo.png' );
					break;
			}
		}
