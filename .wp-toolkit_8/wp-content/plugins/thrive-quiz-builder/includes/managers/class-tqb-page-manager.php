...
class TQB_Page_Manager {
	 ...
	 *
	 * @param array $variation Variation data including content and quiz_id.
	 * @param array $points    Points data including user_points, result_id, category_breakdown, and explicit result.
	 *
	 * @return string The variation content with shortcodes replaced.
	 */
	public static function insert_result_shortcodes( $variation, $points ) {

		// Validate parameters
		if ( ! is_array( $variation ) || ! is_array( $points ) ) {
			return isset( $variation['content'] ) ? $variation['content'] : '';
		}

		// Ensure required keys exist
		if ( ! isset( $variation['quiz_id'], $variation['content'] ) ) {
			return isset( $variation['content'] ) ? $variation['content'] : '';
		}
		...
		// Sanitize category_breakdown - cast IDs and points to integers
		$raw_breakdown = isset( $points['category_breakdown'] ) && is_array( $points['category_breakdown'] )
			? $points['category_breakdown']
			: array();

		$category_breakdown = array();
		foreach ( $raw_breakdown as $cat ) {
			if ( ! isset( $cat['result_id'], $cat['user_points'] ) ) {
				continue;
			}
			$category_breakdown[] = array(
				'result_id'   => (int) $cat['result_id'],
				'user_points' => (int) $cat['user_points'],
			);
		}

		// Replace the original category_breakdown with the sanitized version.
		$points['category_breakdown'] = $category_breakdown;

		// Get the winning result for backward compatibility with [tqb_quiz_result] shortcode
		$result = self::determine_winning_result( $points );

		$data = array(
			'points'    => $points,
			'quiz_type' => sanitize_key( $type ),
			'quiz_id'   => (int) $variation['quiz_id'],
			'result'    => $result,
		);
		...
		// Inject data attribute into any tqb_quiz_result shortcode that doesn't have it
		$data_encoded = base64_encode( wp_json_encode( $data ) );
		$variation['content'] = preg_replace_callback(
			'/\[tqb_quiz_result(?![^\]]*\bdata=)([^\]]*)\]/',
			function( $matches ) use ( $data_encoded ) {
				$attrs = $matches[1];
				return '[tqb_quiz_result' . $attrs . " data='" . $data_encoded . "']";
			},
			$variation['content']
		);
	...
	/**
	 * Determine the winning result for backward compatibility
	 *
	 * @since 3.x.x
	 *
	 * @param array $points Points data.
	 *
	 * @return mixed Result value (string, number, or empty string).
	 */
	private static function determine_winning_result( $points ) {

		if ( ! empty( $points['explicit'] ) ) {
			return $points['explicit'];
		}

		if ( ! isset( $points['result_id'] ) || (int) $points['result_id'] <= 0 ) {
			return isset( $points['user_points'] ) ? $points['user_points'] : '';
		}
		...
		$result_data = $tqbdb->get_quiz_results_single( $points['result_id'] );

		return ! empty( $result_data['text'] ) ? $result_data['text'] : '';
	}
	...
	public static function get_result_shortcodes( $data ) {
		...
		// Use base64 encoding to avoid JSON parsing issues in shortcode attributes.
		// No need for COMMA_PLACEHOLDER transform — base64 safely handles all characters.
		$data_encoded = base64_encode( wp_json_encode( $data ) );

		return array(
			'whole_number' => array(
				'initial'     => "[tqb_quiz_result result_type='whole_number']",
				'replacement' => "[tqb_quiz_result result_type='whole_number' data='" . $data_encoded . "']",
			),
			'one_decimal'  => array(
				'initial'     => "[tqb_quiz_result result_type='one_decimal']",
				'replacement' => "[tqb_quiz_result result_type='one_decimal' data='" . $data_encoded . "']",
			),
			'two_decimal'  => array(
				'initial'     => "[tqb_quiz_result result_type='two_decimal']",
				'replacement' => "[tqb_quiz_result result_type='two_decimal' data='" . $data_encoded . "']",
			),
			'default'      => array(
				'initial'     => "[tqb_quiz_result result_type='default']",
				'replacement' => "[tqb_quiz_result result_type='default' data='" . $data_encoded . "']",
			),
		);
	}
