...
class TQB_Frontend_Ajax_Controller {
	...
	protected function shortcode_action() {
		...
		if ( ! empty( $custom_action ) ) {
			return $this->handle_custom_action( $custom_action );
		}
	 ...
	 * @return mixed
	 */
	protected function handle_custom_action( $action ) {
		switch ( $action ) {
			case 'log_social_share_conversion':
				return $this->log_social_share_conversion();

			case 'save_user_custom_social_share_badge':
				return $this->save_user_custom_social_share_badge();

			case 'register_question_answer':
				return $this->register_question_answer();
	 ...
	 * @return bool
	 */
	protected function log_social_share_conversion() {
		do_action( 'tqb_register_social_media_conversion', $_POST ); // phpcs:ignore
		return true;
	}

	/**
	 * Saves user custom social share badge
	 *
	 * @return string Badge URL with cache buster
	 */
	protected function save_user_custom_social_share_badge() {
		...
		do_action( 'tqb_generate_user_social_badge_link', $user_id, $url );
	 ...
	 * @return bool|array
	 */
	protected function register_question_answer() {
		...
		if ( ! is_array( $answer_ids ) ) {
			$answer_ids = array_filter( array( $answer_ids ) );
		}
		...
		// Get feedback settings with caching (1 hour TTL)
		$cache_key = 'tqb_feedback_settings_' . $quiz_id;
		$feedback_settings = get_transient( $cache_key );
		if ( false === $feedback_settings ) {
			$feedback_settings = get_post_meta( $quiz_id, 'tqb_quiz_feedback_settings', true );
			if ( ! is_array( $feedback_settings ) ) {
				$feedback_settings = array();
			}
			set_transient( $cache_key, $feedback_settings, HOUR_IN_SECONDS );
		}

		//if quiz navigation was enabled then we might get the same or different answers to an already answered question
		if ( ! empty( $feedback_settings['nav_buttons'] ) ) {
			TQB_Quiz_Manager::handle_answer_already_saved( $answer_ids, $quiz_user_id, $quiz_id );
		}

		// Register answers individually to ensure all multi-select answers are saved correctly
		foreach ( $answer_ids as $answer_id ) {
			TQB_Quiz_Manager::register_answer( $answer_id, $user_unique, $quiz_id, $answer_text );
		}

		// Build question content and register impression
		...
		// Return resumed quiz data if navigation is enabled
		if ( ! empty( $feedback_settings['nav_buttons'] ) ) {
	...
	/**
	 * Prewarms Twitter share URL
	 *
	 * @return bool
	 */
	protected function prewarm_twitter_share() {
		$share_url = esc_url_raw( $this->param( 'share_url' ) );

		if ( empty( $share_url ) ) {
			return false;
		}

		wp_remote_head( $share_url, array(
			'timeout'    => 5,
			'user-agent' => 'Twitterbot/1.0 (compatible; TQB Prewarm)',
			'headers'    => array(
				'Accept' => 'text/html',
			),
			'sslverify'  => false,
	...
	/**
	 * Handles GET request for shortcode
	 *
	 * @return array|string
	 */
	protected function handle_get_request() {
		...
		// Handle TCB editor rendering
		if ( $in_tcb_editor === 'inside_tcb' && ( empty( $qna[ $quiz_style ] ) || $data['page'] ) ) {
			return tqb_render_shortcode( array(
				'quiz_id'       => $quiz_id,
				'in_tcb_editor' => $in_tcb_editor,
			) );
		}

		// Process QnA templates if available
		if ( ! empty( $qna[ $quiz_style ] ) ) {
			$data = $this->process_qna_templates( $data, $qna[ $quiz_style ], $quiz_id, $in_tcb_editor );
		}
	...
	/**
	 * Processes QnA templates
	 *
	 * @param array  $data          Quiz data.
	 * @param array  $qna_templates QnA templates.
	 * @param int    $quiz_id       Quiz ID.
	 * @param string $in_tcb_editor TCB editor flag.
	 *
	 * @return array Modified data.
	 */
	protected function process_qna_templates( $data, $qna_templates, $quiz_id, $in_tcb_editor ) {
		$data['qna_templates'] = $qna_templates;
		...
		// Add media display if in editor mode
