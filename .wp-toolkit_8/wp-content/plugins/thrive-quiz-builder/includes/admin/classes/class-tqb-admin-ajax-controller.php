...
class TQB_Admin_Ajax_Controller {
		...
		if ( ( $custom = $this->param( 'custom' ) ) ) {
			switch ( $custom ) {
				...
				case 'anonymize_results':
					...
					break;
				case 'check_optin_connections':
					$quiz_id = $this->param( 'quiz_id', 0 );
					$page_id = $this->param( 'page_id', 0 );
					return $this->check_optin_gate_connections( $quiz_id, $page_id );
					break;
				default:
					return array();
			}
	...
	protected function quizzes_action() {
		...
		switch ( $method ) {
			...
			case 'GET':
				...
				$include_statistics = $this->param( 'include_statistics' ) !== '0'; // Default to true unless explicitly set to '0'

				if ( is_numeric( $all_quizzes ) ) {
					return TQB_Quiz_Manager::get_quizzes( array(), true, $include_statistics );
					...
					return TQB_Quiz_Manager::get_specific_quizzes( $quizzes_id, $include_statistics );
				} else if ( ! empty( $search_title ) ) {
					return TQB_Quiz_Manager::get_searched_quizzes( $search_title, $include_statistics );
				}
	 ...
	 * @return array
	 */
	protected function quizstatistics_action() {
		$quiz_ids = $this->param( 'quiz_ids' );

		if ( empty( $quiz_ids ) || ! is_array( $quiz_ids ) ) {
			return array();
		}

		return TQB_Quiz_Manager::get_quizzes_statistics( $quiz_ids );
	}
	...
	protected function settings_action() {
		...
		$method         = empty( $_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'] ) ? $request_method : sanitize_text_field( wp_unslash( $_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'] ) );

		switch ( $method ) {
			case 'POST':
			case 'PUT':
			case 'PATCH':
				// Early exit: Validate model data
				if ( ! is_array( $model ) ) {
					return array();
				}

				// Special handling for 'tqb_enable_stats_tracking'
				// If not present in the model, it means it's ON, so remove it from saved settings
				$old_settings = tqb_get_option( Thrive_Quiz_Builder::PLUGIN_SETTINGS, array() );
				if ( ! isset( $model['tqb_enable_stats_tracking'] ) && isset( $old_settings['tqb_enable_stats_tracking'] ) ) {
					// Remove the key from old settings before merging
					unset( $old_settings['tqb_enable_stats_tracking'] );
					// Merge the new values with the modified old settings
					$model = array_merge( $old_settings, $model );
				}
				// Update using the standard tqb_update_option function for consistency
				tqb_update_option( Thrive_Quiz_Builder::PLUGIN_SETTINGS, $model, true );

				// Return the complete settings (including defaults and merged values)
				return Thrive_Quiz_Builder::get_settings();
				break;
			case 'DELETE':
			case 'GET':
			default:
				error_log( 'TQB_Admin_Ajax_Controller::settings_action received unsupported HTTP method: ' . $method );
				return array(
	...
	/**
	 * Manual trigger for stats cleanup (for testing/debugging)
	 * Allows admin to manually trigger the auto-cleanup process
	 */
	protected function manualcleanupstats_action() {
		// Check user permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			return array( 'error' => 'Insufficient permissions' );
		}

		$result = TQB_Stats_Cleanup_Manager::manual_cleanup();
		$info = TQB_Stats_Cleanup_Manager::get_cleanup_info();
	...
	/**
	 * Check if opt-in gate has lead generation forms with API connections
	 *
	 * @param int $quiz_id The quiz ID
	 * @param int $page_id The opt-in gate page ID
	 * @return array Response array with connection status
	 */
	protected function check_optin_gate_connections( $quiz_id, $page_id ) {
		global $wpdb;

		$response = array(
			'has_optin_gate' => false,
			'has_api_connection' => false,
		);

		if ( empty( $quiz_id ) || ! is_numeric( $quiz_id ) || empty( $page_id ) || ! is_numeric( $page_id ) ) {
			return $response;
		}

		// Query the tqb_variations table
		$table_name = $wpdb->prefix . 'tqb_variations';
		$variation = $wpdb->get_row( $wpdb->prepare(
			"SELECT content FROM {$table_name} WHERE quiz_id = %d AND page_id = %d LIMIT 1",
			$quiz_id,
			$page_id
		) );

		if ( ! $variation || ! isset( $variation->content ) || empty( $variation->content ) ) {
			return $response;
		}

		$response['has_optin_gate'] = true;
		$content = $variation->content;

		// Check if content contains lead generation forms
		if ( strpos( $content, 'thrv_lead_generation' ) === false ) {
			return $response;
		}

		// Find data-form-settings attribute and decode it
		if ( preg_match( '/data-form-settings="([^"]{0,50000})"/', $content, $matches ) ) {
			$form_settings_encoded = $matches[1];
			// Remove __TCB_FORM__ wrappers if present
			$form_settings_encoded = preg_replace('/^__TCB_FORM__|__TCB_FORM__$/', '', $form_settings_encoded);
			// Decode HTML entities and then JSON decode
			$form_settings_json = html_entity_decode( $form_settings_encoded, ENT_QUOTES, 'UTF-8' );
			$form_settings = json_decode( $form_settings_json, true );

			// Check if APIs are configured
			if ( is_array( $form_settings ) && ! empty( $form_settings['apis'] ) && is_array( $form_settings['apis'] ) ) {
				$response['has_api_connection'] = true;
			}
		}
	...
	/**
	 * Export results action - Initiates quiz results export.
	 *
	 * @return array Response array with export mode and details.
	 */
	protected function exportresults_action() {
		global $wpdb;

		if ( ! current_user_can( 'manage_options' ) ) {
			return array(
				'success' => false,
				'message' => __( 'You do not have permission to export quiz results.', 'thrive-quiz-builder' ),
			);
		}

		// Get quiz_id from request.
		$quiz_id = absint( $this->param( 'quiz_id', 0 ) );

		if ( empty( $quiz_id ) ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid quiz ID.', 'thrive-quiz-builder' ),
			);
		}
		...
		if ( ! $quiz_post || TQB_Post_types::QUIZ_POST_TYPE !== $quiz_post->post_type ) {
			return array(
				'success' => false,
				'message' => __( 'Quiz not found.', 'thrive-quiz-builder' ),
			);
		}

		// Count results for this quiz.
		$count = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT COUNT(*) FROM ' . tqb_table_name( 'users' ) . ' WHERE quiz_id = %d AND (ignore_user IS NULL OR ignore_user != 1)',
				$quiz_id
			)
		);
		...
		if ( ! $count || $count < 1 ) {
			return array(
				'success' => false,
				'message' => __( 'No results to export.', 'thrive-quiz-builder' ),
			);
		}

		// Rate limiting: Check cooldown transient (30 seconds).
		$user_id          = get_current_user_id();
		$cooldown_key     = 'tqb_export_cooldown_' . $user_id;
		$cooldown_active  = get_transient( $cooldown_key );

		if ( $cooldown_active ) {
			return array(
				'success' => false,
				'message' => __( 'Please wait before initiating another export.', 'thrive-quiz-builder' ),
			);
		}
		...
		// Check threshold for sync vs async.
		$threshold = TQB_Results_Export_Manager::get_threshold();

		if ( $count <= $threshold ) {
			// Synchronous export - return URL for immediate download.
			$nonce = wp_create_nonce( 'tqb_export_results_' . $quiz_id );
			$url   = add_query_arg(
				array(
					'action'           => 'tqb_export_results',
					'quiz_id'          => $quiz_id,
					'tqb_export_nonce' => $nonce,
				),
				admin_url( 'admin.php' )
			);
		...
		// Asynchronous export - create background job.
		$job_id = TQB_Results_Export_Manager::create_job( $quiz_id );

		if ( ! $job_id ) {
			return array(
				'success' => false,
				'message' => __( 'Failed to create export job.', 'thrive-quiz-builder' ),
			);
		}
	...
	/**
	 * Export results status action - Polls job status for async exports.
	 *
	 * @return array Response array with job status.
	 */
	protected function exportresultsstatus_action() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return array(
				'success' => false,
				'message' => __( 'You do not have permission to check export status.', 'thrive-quiz-builder' ),
			);
		}
		...
		if ( empty( $job_id ) ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid job ID.', 'thrive-quiz-builder' ),
			);
		}

		// Get job status.
		$job_data = TQB_Results_Export_Manager::get_job_status( $job_id );

		if ( ! $job_data ) {
			return array(
				'success' => false,
				'message' => __( 'Export job not found.', 'thrive-quiz-builder' ),
			);
		}

		// Validate ownership - already done in get_job_status.
		$status = isset( $job_data['status'] ) ? $job_data['status'] : 'unknown';

		switch ( $status ) {
			case 'completed':
				// Generate download URL with nonce.
				$nonce = wp_create_nonce( 'tqb_download_export_' . $job_id );
				$url   = add_query_arg(
					array(
						'action' => 'tqb_download_export',
						'job_id' => $job_id,
						'nonce'  => $nonce,
					),
					admin_url( 'admin.php' )
				...
				$row_count = isset( $job_data['row_count'] ) ? $job_data['row_count'] : 0;
				...
				return array(
					...
					'url'     => $url,
					'message' => sprintf(
						__( 'Export complete. %d results exported.', 'thrive-quiz-builder' ),
						$row_count
					),
				);

			case 'failed':
				$error = isset( $job_data['error'] ) ? $job_data['error'] : __( 'Unknown error.', 'thrive-quiz-builder' );
				return array(
					'success' => true,
					'status'  => 'failed',
					'message' => sprintf(
						__( 'Export failed: %s', 'thrive-quiz-builder' ),
						$error
					),
				);
			...
			default:
				return array(
					'success' => true,
					'status'  => 'processing',
					'message' => __( 'Export in progress...', 'thrive-quiz-builder' ),
				);
		}
	}
}
