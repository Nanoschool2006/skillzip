...
class TVA_User_Assessment implements JsonSerializable {
	...
	const ASSESSMENT_GRADED = 'tva_assessment_graded_';

	const ASSESSMENT_PASSED = 'tva_assessment_passed_';
	...
	public function create() {
		...
		$completion_credit_type = Grading_Base::get_assessment_completion_credit( $this->post_parent );
		 ...
		 */
		if ( Grading_Base::CREDIT_ON_SUBMIT === $completion_credit_type ) {
			$this->increase_user_submission_counter();
			$this->maybe_add_course_completed_meta( $completion_credit_type );
		}
	...
	public function get_user_assessment_counter( $meta_key ) {

		return (int) get_user_meta( $this->post_author, $meta_key . $this->post_parent, true ) ?: 0;
	}

	public function increase_user_assessment_counter( $meta_key ) {
		$counter = $this->get_user_assessment_counter( $meta_key ) + 1;

		update_user_meta( $this->post_author, $meta_key . $this->post_parent, $counter );
	}
	...
	public function set_course_id() {
		...
		if ( $this->post_parent ) {
			$assessment = new TVA_Assessment( $this->post_parent );

			if ( $assessment->get_course_v2() instanceof TVA_Course_V2 ) {
				update_post_meta( $this->ID, static::COURSE_META, $assessment->get_course_v2()->get_id() );
			}
		}
	}
	...
	/**
	 * @return string The name of the course associated with this user assessment
	 */
	...
	public static function delete_user_submissions( $assessment_id, $user_id = 0 ) {
		...
		delete_user_meta( $user_id, static::ASSESSMENT_GRADED . $assessment_id );
		delete_user_meta( $user_id, static::ASSESSMENT_PASSED . $assessment_id );
	}
	 ...
	 * @param string     $notes
	 * @param string     $completion_credit
	 *
	 * @return bool Whether the grading was successful or not
	 */
	public function save_grade( $grade, $notes, $completion_credit = Grading_Base::CREDIT_ON_SUBMIT ) {
		$grading_details = Grading_Base::get_assessment_grading_details( $this->post_parent );
		...
		// Set assessment as complete if $completion_credit is either upon_passing or upon_grading.
		switch ( $completion_credit ) {
			case Grading_Base::CREDIT_ON_PASS:
				// Completed only if user passed.
				if ( $passed ) {
					$this->increase_user_assessment_counter( static::ASSESSMENT_PASSED );
					$this->maybe_add_course_completed_meta( $completion_credit );
				}

				break;
			case Grading_Base::CREDIT_ON_GRADE:
				$this->increase_user_assessment_counter( static::ASSESSMENT_GRADED );
				$this->maybe_add_course_completed_meta( $completion_credit );
	...
	/**
	 * Maybe add user meta for course completion after grading.
	 * 
	 * @param string $completion_credit
	 *
	 * @return void
	 */
	public function maybe_add_course_completed_meta( $completion_credit ) {
		$customer = new TVA_Customer( $this->post_author );
		$course_id = $this->get_course_id();

		if ( $customer->has_completed_course( $course_id ) ) {
			$customer::add_user_completed_meta( $this->post_author, $course_id );
