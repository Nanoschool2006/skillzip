...
function tva_send_hooks_for_item( $lesson_id, $state = 'start', $user_id = null ) {
	...
	$course_items_count = (int) $course->get_published_lessons_count() + (int) $course->get_published_assessments_count();
	$completed_items_count = (int) $customer->get_count_course_completed_lessons( $course->get_id() ) +
	                         (int) $customer->get_user_assessments_completed( $course );

	if ( $state === 'start' && $course_items_count === 1 ) {
...
function tva_send_hooks( $lesson_id, $state = 'start', $course_v2 = null ) {
	...
	if ( ! $course_found ) {
		...
		$course_cookie_name = 'course_' . $lesson_course_id . '_started';
		$customer              = new TVA_Customer( $user_id );
		$course_items_count    = (int) $course_v2->get_published_lessons_count() + (int) $course_v2->get_published_assessments_count();
		$completed_items_count = (int) $customer->get_count_course_completed_lessons( $course_v2->get_id() ) +
								 (int) $customer->get_user_assessments_completed( $course_v2 );
		...
		if ( $state === 'start' ) {
			...
			$state === 'end' && TVA_Cookie_Manager::get_cookie( $course_cookie_name ) &&
			$completed_items_count === $course_items_count
		) {
			 ...
			 *
			 * This hook affects the course completion tracking.
			 *
			 * @param array Course Details
			 * @param null|array User Details
			 *
...
function tva_add_head_styles() {
	...
	if ( tva_is_apprentice() ) {
		...
		<?php if ( 'google' === $template_settings['font_source'] ) : ?>
			<?php
			$font_url = $template_settings['font_url'] ? $template_settings['font_url'] : '';
			if ( tve_dash_is_google_fonts_blocked() ) {
				$font_url = tve_replace_google_font_with_bunny( $font_url );
			}
			?>
...
/**
 * Get WP gmt_offset option and return a formatted UTC string.
 *
 * @return string
 */
function format_gmt_offset() {
    ...
    $fraction = abs( $offset ) - $hours;
    $minutes = (int) ( $fraction * 60 );

    // Format the string with leading sign, hours, and minutes.
...
/**
 * Get course progress for current user using centralized logic
 *
 * @param int|TVA_Course_V2 $course Course ID or Course object
 * @return string Progress percentage (e.g., "75%")
 */
function tva_get_course_progress( $course, $user_id = null ) {
    ...
    $progress_data = $customer->calculate_progress( $course );
